...
Enquanto no mundo relacional, o setor de software tem muita experiência em como modelar dados, ainda estamos nos estágios iniciais do espaço de repositório de conteúdo.

Eu gostaria de começar a preencher esse vazio expressando minhas opiniões sobre como o conteúdo deve ser modelado, esperando que isso possa algum dia se formar em algo mais significativo para a comunidade de desenvolvedores, que não é apenas &quot;minha opinião&quot;, mas algo que é mais geralmente aplicável. Então considerem isso como minha primeira facada em rápida evolução.
...
Eu acho que Stefano resume bem essa.
...
A hierarquia de conteúdo é um ativo valioso. Então, não apenas deixe acontecer, desenhe-a. Se você não tem um nome &quot;bom&quot; em formato legível por humanos para um nó, isso provavelmente é algo que você deveria reconsiderar. Números arbitrários quase nunca são um &quot;bom nome&quot;.
...
Pessoalmente, prefiro convenções de hierarquia sobre o sistema de digitação de nós em muitos casos inicialmente, e introduzo a digitação mais tarde.
...
Eu modelaria um simples sistema de blogues da seguinte maneira. Observe que inicialmente eu nem me importo com os respectivos tipos de nós que eu uso neste momento.
...
Embora o Same Name Irmãos (SNS) tenha sido introduzido na especificação para permitir compatibilidade com estruturas de dados projetadas para e expressas por XML e, portanto, são valiosas para JCR, o SNS vem com uma sobrecarga e complexidade substanciais para o repositório.
...
Pessoalmente, eu só uso referências quando realmente não consigo lidar com uma referência pendente e, caso contrário, uso um caminho, um nome ou uma UUID de sequência para fazer referência a outro nó.
...
Eu acho que como regra geral pode-se usar o seguinte: Se você deve armazenar o nome do arquivo e o tipo MIME então `nt:file`/ `nt:resource` O é um bom jogo. Se você tiver vários &quot;arquivos&quot;, nt:folder é um bom lugar para armazená-los.
...
Embora certamente haja bons casos de uso para usar apenas uma propriedade binária (digamos que o nome seja irrelevante e o tipo MIME esteja implícito), neste caso, recomendo a seguinte estrutura para meu exemplo de blog.
...
É verdade que alguns nós precisam de uma identificação estável ao longo de seu ciclo de vida. Muito menos do que você imagina. mix:referenceable fornece esse mecanismo integrado no repositório, de modo que não há necessidade de criar um meio extra de identificar um nó de maneira estável.
...
Portanto, digamos que você gostaria de poder fazer referência a um nó do tipo &quot;Documento&quot; não signifique que o tipo de nó &quot;Documento&quot; precise se estender de mix:referenceable de forma estática, pois ele pode ser adicionado a qualquer instância do &quot;Documento&quot; dinamicamente.
