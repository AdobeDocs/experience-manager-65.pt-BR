...
* **Container**
   ...
   * **Sub fluxo de trabalho**: Selecione o fluxo de trabalho a ser start.
...
* Se a condição especificada for verdadeira, a **Etapa Ir para** será concluída e o motor de workflow executará a etapa especificada.
* Se a condição especificada não for verdadeira, a **Etapa de Ir para** será concluída e a lógica normal do roteamento determinará a próxima etapa a ser executada.
...
Por exemplo, para implementar um fluxo de trabalho que executa uma ação em vários nós JCR, você pode usar um contador de loop como índice para os nós. Para persistir na contagem, armazene um valor `integer` no mapa de dados da instância do fluxo de trabalho. Use o script de **Ir para a Etapa** para incrementar a contagem, bem como para comparar a contagem com os critérios de saída.
...
Também é possível simular um loop for usando a Definição de regra como a expressão do roteamento. [Crie uma variável  **** ](/help/forms/using/variable-in-aem-workflows.md#create-a-variable) contável de tipo de dados Longo. Use **Expressão** como o modo de mapeamento na etapa **[Definir Variável](/help/sites-developing/using-variables-in-aem-workflows.md#set-a-variable)** para definir o valor da variável **count** como **count + 1** em cada execução da etapa **Definir Variável**.
...
* Edite as **OU propriedades de divisão**:
   ...
   * **Ramificações (*x)***
      ...
      * **Selecione a Expressão** do Roteamento: Selecione a expressão do roteamento para avaliar a ramificação ativa. Os valores possíveis incluem: Definição de regra, Script externo e script ECMA.
      * **Clique para Adicionar Expressão**: Adicione expressão para avaliar a ramificação ativa se você selecionar  **Definição de** regra na expressão do roteamento.
      * **Caminho** do script: O caminho para um arquivo que contém o script para avaliar a ramificação ativa se você selecionar  **Scripts externos** como a expressão do roteamento.
      * **Script**: Adicione o script na caixa para avaliar a ramificação ativa se você selecionar  **ECMA** Scriptas na expressão do roteamento.
   ...
   >[!NOTE]
   >
   >    * Uma ramificação é avaliada de cada vez com base na expressão do roteamento.
...
1. **Propriedade do caminho de diálogo**
   ...
   A **Etapa do participante da caixa de diálogo** tem a propriedade **Caminho da caixa de diálogo** (juntamente com as propriedades de uma [Etapa do participante](#participant-step)). O valor da propriedade **Dialog Path** é o caminho para o nó `dialog` da sua caixa de diálogo.
...
* **Serviço OSGi**
   ...
   * `getParticipant` método: Retorna a ID Principal dinamicamente resolvida como um  `String` valor.
   ...
   >[!NOTE]
   >
   >**Random Participant** Chooseris é um serviço de amostra que seleciona um usuário aleatório (  `com.day.cq.workflow.impl.process.RandomParticipantChooser`). A amostra do componente **Random Participant Choose** r Step estende a **Etapa dinâmica do participante** e usa esse serviço como a implementação da etapa.
...
Na caixa de diálogo de propriedades **Etapa dinâmica do participante**, a lista **Seletor de participantes** inclui o item `Workflow Initiator Participant Chooser (script)`, que representa este serviço.

Quando o modelo de fluxo de trabalho é iniciado, o log indica a ID do usuário que iniciou o fluxo de trabalho e quem recebeu o item de trabalho. Neste exemplo, o usuário `admin` iniciou o fluxo de trabalho.
