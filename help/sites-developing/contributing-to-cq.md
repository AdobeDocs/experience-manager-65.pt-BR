...
Projetar para compatibilidade com versões anteriores também garante que a experiência de atualização corresponda à simplicidade da instalação inicial. Parar o AEM, substituir o arquivo JAR do Quickstart e iniciar o AEM novamente devem ser suficientes. Com uma base de instalação em rápido crescimento, a eficiência do upgrade é uma vantagem cada vez mais significativa.
...
**Componente** - No AEM, um Componente é um tipo de objeto, cujas instâncias geralmente podem ser criadas arrastando-as e soltando-as, por exemplo, no Sidekick. Por exemplo, os componentes prontos para uso que acompanham o AEM incluem os componentes Texto, Título, Tag Cloud, Carrossel, Imagem e Lista, todos disponíveis no Sidekick no tempo de execução.

**Localizador de Conteúdo** - No modo de criação, o Localizador de Conteúdo é um painel especial (quadro) no lado esquerdo da página que, dependendo da guia selecionada na parte superior, exibe listas de imagens, documentos, ativos de Flash, páginas, parágrafos ou recursos de repositório que você pode arrastar e soltar do Localizador de Conteúdo na página em que está trabalhando (à direita).
...
**Widgets ExtJS** - A maioria dos elementos da interface de usuário no AEM usa ExtJS, que é uma biblioteca de widgets de terceiros gravada no JavaScript. O ExtJS apresenta widgets de interface do usuário personalizáveis de alto desempenho e um modelo de componentes extensível e bem projetado.
...
**Microkernel** - Cada espaço de trabalho no repositório pode ser configurado separadamente para armazenar seus dados por meio de um microkernel específico (uma classe que gerencia a leitura e a gravação dos dados). Da mesma forma, o armazenamento de versão em todo o repositório também pode ser configurado independentemente para usar um microkernel específico. Vários microkernels diferentes estão disponíveis, capazes de armazenar dados em vários formatos de arquivo ou bancos de dados relacionais. (Por exemplo, há gerenciadores de persistência para MongoDB, DB2® ou Oracle) O microkernel padrão para AEM é TarMK (veja mais abaixo).

**Instância do Publish** - Por motivos de segurança, governança e outros, um site de produção normalmente dividirá instâncias de AEM em instâncias de Autor e do Publish. Para obter mais informações sobre a arquitetura de implantação (incluindo instâncias de Autor/Publish), consulte a documentação sobre Instâncias AEM.
...
Muitas opções de inicialização (como o número da porta ativa e se a instância do AEM em questão deve ser uma instância do Publish versus uma instância do Author e muito mais) podem ser controladas renomeando adequadamente o arquivo Quickstart. Para ver uma lista de opções a esse respeito, execute o JAR com &quot;-help&quot; na linha de comando:

```shell
java -jar <quickstartfilename>.jar -help
```

**Agentes de replicação** - Os agentes de replicação são fundamentais para o AEM como o mecanismo usado para ativar o conteúdo do Publish de um autor para um ambiente de publicação; liberar conteúdo do cache do Dispatcher; retornar conteúdo gerado pelo usuário (por exemplo, entrada de formulário) do ambiente do Publish para o ambiente do Autor.
...
**Sidekick** - O Sidekick é uma janela flutuante semelhante a uma paleta que aparece na página editável, da qual novos componentes podem ser arrastados e as ações que se aplicam à página podem ser executadas.

**Site Catalyst** - o SiteCatalyst fornece aos profissionais de marketing um local para medir, analisar e otimizar os dados integrados de todas as iniciativas online em vários canais de marketing. Você pode usar o Adobe SiteCatalyst para analisar dados de sites AEM.

**Armazenamento de Tar (TarMK)** - TarMK é o sistema de persistência padrão no AEM. Embora o AEM possa ser configurado para usar um sistema de persistência diferente (como o MongoDB), o TarMK tem certas vantagens, pois é otimizado para desempenho em casos de uso típicos de JCR (portanto, é rápido), usa um formato de dados padrão do setor e pode ter backup rápido e fácil.
