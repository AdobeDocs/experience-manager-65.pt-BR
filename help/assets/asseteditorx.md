...
[!DNL Assets] os componentes usam uma extensão da clientlib de edição do WCM. Normalmente, os clientlibs são carregados em `init.jsp`.

Em comparação com o carregamento padrão clientlib (no núcleo `init.jsp`), um modelo [!DNL Assets] deve ter o seguinte:
...
Alguns dos componentes [!DNL Assets] exigem funções JS definidas em `component.js`. Copie esse arquivo para o diretório do componente e vincule-o.
...
Alguns dos componentes [!DNL Assets] usam a biblioteca de widgets. Para ser renderizada corretamente no contexto do conteúdo, uma folha de estilos adicional deve ser carregada. O componente de ação de tag requer mais um.
