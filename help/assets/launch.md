...
O caso de uso principal para a integração com o Adobe Launch são os clientes que usam os ativos AEM e os sites AEM. Nesses cenários, você pode configurar uma integração padrão entre o nó do autor do AEM e o Adobe Launch e associar a instância do Sites à propriedade do Adobe Launch. Depois disso, qualquer componente WCM do Dynamic Media adicionado a uma página Sites rastreará os dados e eventos dos visualizadores.
...
* A extensão do Adobe Analytics estende a lista de ações compatíveis com duas ações necessárias para enviar dados aos servidores de rastreamento: *Defina as variáveis* e *envie o beacon*.
...
Os clientes que não usam o AEM Sites ou incorporam visualizadores de Mídia dinâmica em páginas da Web fora do AEM Sites, ou ambos, ainda podem usar a integração do Adobe Launch.
...
A extensão Visualizadores de Dynamic Media permite usar parâmetros de evento dos visualizadores de Dynamic Media para mapear para variáveis de análise no editor Definir variáveis da extensão do Adobe Analytics.
...
1. Start acessando o Adobe Analytics a partir do [home page](https://exc-home.experiencecloud.adobe.com/exc-home/home.html#/)da Experience Cloud. Na barra de menus, clique no ícone Soluções (uma tabela de três por três pontos) próximo ao canto superior direito da página e, em seguida, clique em **[!UICONTROL Analytics]**.
...
1. Start acessando o Adobe Launch a partir do [home page](https://exc-home.experiencecloud.adobe.com/exc-home/home.html#/)da Experience Cloud. Na barra de menus, clique no ícone Soluções (três por três tabelas de pontos) próximo ao canto superior direito da página e clique em **[!UICONTROL Iniciar]**.
...
Preencha o campo do servidor de rastreamento. O servidor de rastreamento para a extensão de Áudio e Vídeo *do* Adobe Media Analytics é diferente do servidor de rastreamento usado no Adobe Analytics. Ele segue o modelo `<trackingNamespace>.hb.omtrdc.net`, onde `<trackingNamespace>` estão as informações do email de provisionamento.
...
1. Na terceira página **[!UICONTROL Criar uma nova integração]** , faça o seguinte:
   ...
   * Em **[!UICONTROL Selecionar um ou mais perfis de produto para a API]** de lançamento da plataforma Experience, selecione o perfil de produto chamado **[!UICONTROL Iniciar - &lt;nome_empresa_do_produto>]**.
