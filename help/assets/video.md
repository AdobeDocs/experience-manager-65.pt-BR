...
>[!NOTE]
>
>Antes de trabalhar com vídeo no Dynamic Media, verifique se o administrador do AEM já habilitou e configurou os Serviços do Dynamic Media Cloud no modo Mídia dinâmica - Scene7 ou Mídia dinâmica - Híbrido.
>
>* Consulte [Configuração dos serviços](/help/assets/config-dms7.md#configuring-dynamic-media-cloud-services) da Dynamic Media Cloud em Configuração do Dynamic Media - modo Scene7 e [Solução de problemas do Dynamic Media - modo Scene7.](/help/assets/troubleshoot-dms7.md)
   >
   >
* Consulte [Configuração dos serviços](/help/assets/config-dynamic.md#configuring-dynamic-media-cloud-services) da Dynamic Media Cloud em Configuração do Dynamic Media - Modo híbrido.
>
...
1. **Carregue seus vídeos** do Dynamic Media fazendo o seguinte:
   ...
   * Associe o perfil de processamento de vídeo a uma ou mais pastas onde você carregará seus vídeos mestre.
      ...
      * Saiba mais sobre as práticas [recomendadas para organizar seus ativos digitais para usar perfis](/help/assets/organize-assets.md)de processamento.
...
1. **Gerencie seus vídeos** do Dynamic Media executando um dos procedimentos a seguir:
   ...
   * Organizar, navegar e pesquisar ativos de vídeo
      ...
      * [Organização de ativos](/help/assets/organize-assets.md)digitais Saiba mais sobre as práticas [recomendadas para organizar seus ativos digitais para usar perfis de processamento](organize-assets.md)
   ...
   * Revise, aprove e anote vídeos e mantenha o controle total da versão
      ...
      * [Aplicar fluxos de trabalho a ativos](assets-workflow.md) ou consulte [Iniciar um fluxo de trabalho em um ativo](managing-assets-touch-ui.md#starting-a-workflow-on-an-asset)
...
Quando você ignora o perfil de processamento de vídeo e publica diretamente no YouTube, isso significa apenas que seu ativo de vídeo no AEM Asset pode não obter uma miniatura visível. Isso também significa que se você executar nos modos de execução dynamicmedia ou dynamicmedia_sceno7, os vídeos que não estão codificados não funcionarão com nenhum dos tipos de ativos de Mídia dinâmica.
...
* Para configurar o YouTube no AEM antes de 6.4, consulte [Configuração do YouTube no AEM antes de 6.4](/help/assets/video.md#setting-up-youtube-in-aem-before).
...
1. Aplique o perfil de metadados de publicação do YouTube às pastas onde você carregará vídeos. Será necessário definir o Perfil de metadados e o Perfil de vídeo.
   ...
   Consulte Perfis [de](/help/assets/metadata-profiles.md) metadados e perfis [de](/help/assets/video-profiles.md)vídeo.
...
>[!NOTE]
>
>Durante a execução no modo Mídia dinâmica - Scene7, observe que a publicação imediata não é publicada automaticamente no YouTube. Quando o modo Dynamic Media - Scene7 estiver configurado, há duas opções de publicação a serem escolhidas: **[!UICONTROL Imediatamente]** ou **[!UICONTROL Após Ativação]**.
>
...
1. No código incorporado, localize a linha com o seguinte:
   ...
   O valor correto para o parâmetro config2 é encontrado na função **Embed Code **e na função copy **URL **function. No URL do comando copy **URL **, o parâmetro a ser procurado é `&config2=<value>` . O valor é quase sempre `companypreset`, mas em alguns casos também pode ser `companypreset-1`, `companypreset-2`e assim por diante.
...
1. No modo de seleção de ativo na Exibição **[!UICONTROL de]** lista ou na Exibição **[!UICONTROL de]** cartão, toque no ativo de vídeo.
...
1. No modo de seleção de ativos, na Exibição de lista ou na Exibição de cartão, toque no ativo de vídeo.
