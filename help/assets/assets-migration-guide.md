...
1. Execute uma pesquisa de sistema de arquivos em `crx-quickstart/launchpad/felix` Procurar `sling.id`. Exclua este arquivo.
...
1. Desativar fluxos de trabalho:Como você está migrando execuções juntamente com nossos ativos, deseja desativar os iniciadores de fluxo de trabalho para o Ativo de atualização do DAM.

1. Migrar tags:Como as tags já foram carregadas na instância de AEM de origem, é possível criá-las em um pacote de conteúdo e instalá-las na instância de destino.
...
1. Ativar fluxos de trabalho:Após concluir a migração, ative novamente os iniciadores para os fluxos de trabalho do Ativo de atualização do DAM para suportar a geração de execução e a extração de metadados para uso diário do sistema.
