...
description: Descreve como transferir ativos para o [!DNL Adobe Experience Manager], aplique metadados, gere representações e ative-as para publicar instâncias.
contentOwner: AG
...
Ao migrar ativos para [!DNL Adobe Experience Manager], há várias etapas a serem consideradas. A extração de ativos e metadados para fora de sua casa atual está fora do escopo deste documento, pois varia muito entre implementações, mas este documento descreve como trazer esses ativos para [!DNL Experience Manager], aplicar os metadados, gerar representações e ativá-las para publicar instâncias.

## Pré-requisitos {#prerequisites}

Antes de realmente executar qualquer uma das etapas desta metodologia, analise e implemente as orientações em [Dicas de ajuste de desempenho de ativos](performance-tuning-guidelines.md). Muitas etapas, como a configuração de trabalhos simultâneos máximos, melhoram muito a estabilidade e o desempenho do servidor sob carga. Outras etapas, como configurar um File Data Store, são muito mais difíceis de executar depois que o sistema é carregado com ativos.
...
Antes de iniciar a migração, desative os lançadores para a [!UICONTROL Ativo de atualização DAM] fluxo de trabalho. É melhor assimilar todos os ativos no sistema e, em seguida, executar os workflows em lotes. Se você já estiver ao vivo enquanto a migração estiver ocorrendo, poderá agendar essas atividades para serem executadas fora do horário.

### Carregar tags {#loading-tags}

É possível que você já tenha uma taxonomia de tags em vigor que esteja aplicando às imagens. Ferramentas como o Importador de ativos CSV e [!DNL Experience Manager] o suporte a perfis de metadados pode automatizar o processo de aplicação de tags a ativos, as tags precisam ser carregadas no sistema. O [Criador de tags de ferramentas AEM ACS](https://adobe-consulting-services.github.io/acs-aem-tools/features/tag-maker/index.html) permite que você preencha as tags usando uma planilha do Excel do Microsoft carregada no sistema.
...
A equipe do Managed Services do Adobe usa uma ferramenta chamada Glutton para carregar dados em ambientes do cliente. O Glutton é um pequeno aplicativo Java que carrega todos os ativos de um diretório em outro diretório em um [!DNL Experience Manager] implantação. Em vez do Glutton, você também pode usar ferramentas como scripts Perl para publicar os ativos no repositório.
...
Depois de carregar os ativos no sistema, você precisa processá-los por meio do [!UICONTROL Ativo de atualização DAM] fluxo de trabalho para extrair metadados e gerar representações. Antes de executar essa etapa, é necessário duplicar e modificar o [!UICONTROL Ativo de atualização DAM] fluxo de trabalho para atender às suas necessidades. O fluxo de trabalho pronto para uso contém várias etapas que podem não ser necessárias para você, como a geração de PTIFF do Dynamic Media ou [!DNL InDesign Server] integração.
...
Depois de concluir a migração, os lançadores da [!UICONTROL Ativo de atualização DAM] os workflows devem ser reativados para oferecer suporte à geração de representação e extração de metadados para uso diário do sistema.

## Migrar entre [!DNL Experience Manager] implantações {#migrating-between-aem-instances}

Embora não seja quase tão comum, às vezes você precisa migrar grandes quantidades de dados de um [!DNL Experience Manager] Implantação noutro; por exemplo, ao executar um [!DNL Experience Manager] atualizar, atualizar seu hardware ou migrar para um novo data center, como com uma migração do AMS.

Nesse caso, seus ativos já estão preenchidos com metadados e as representações já são geradas. Você pode simplesmente se concentrar em mover ativos de uma instância para outra. Ao migrar entre [!DNL Experience Manager] , execute as seguintes etapas:

1. Desativar fluxos de trabalho: Como você está migrando representações junto com nossos ativos, deseja desativar os inicializadores do fluxo de trabalho para [!UICONTROL Ativo de atualização DAM] fluxo de trabalho.

1. Migrar tags: Porque você já tem tags carregadas na origem [!DNL Experience Manager] , crie-os em um pacote de conteúdo e instale o pacote na instância de destino.

1. Migrar ativos: Há duas ferramentas recomendadas para mover ativos de um [!DNL Experience Manager] implantação em outro:

   * **Cofre de cópia remota** ou vlt rcp, permite usar vlt em uma rede. Você pode especificar um diretório de origem e de destino e o vlt baixa todos os dados do repositório de uma instância e os carrega na outra. O rcp da vlt está documentado em [https://jackrabbit.apache.org/filevault/rcp.html](https://jackrabbit.apache.org/filevault/rcp.html)
...
1. Ativar ativos: Siga as instruções para [ativação de ativos](#activating-assets) documentado para a migração inicial para o [!DNL Experience Manager].
...
1. Ativar fluxos de trabalho: Após concluir a migração, reative os lançadores para a [!UICONTROL Ativo de atualização DAM] fluxo de trabalho para oferecer suporte à geração de representação e extração de metadados para uso diário do sistema.
