...
| -------- | ---------------------------- |
| Versão | 6.5.23.0, Hotfix para GRANITE-61551 <!-- UPDATE FOR EACH NEW RELEASE --> |
...
* Posicionamento incorreto do botão da Árvore de conteúdo fixo no Editor de páginas. Agora, o Editor de páginas posiciona corretamente o botão de configuração da Árvore de conteúdo na caixa de diálogo &quot;Teaser principal&quot; pretendida, em vez da seção errada. A correção atualiza o CSS para a caixa de diálogo Árvore de Conteúdo para usar `top:0` em vez de `bottom:0`, garantindo o posicionamento adequado do botão. (SITES-28448)
...
Corrija as marcas inesperadas `<br>` no Editor de Rich Text com o modo de colagem de texto sem formatação. O Editor de Rich Text agora lida corretamente com as operações de recorte e colagem ao usar texto sem formatação `defaultPasteMode`. A correção impede a inserção de marcas `<br>` inesperadas quando os usuários recortam e colam texto em campos RTE, garantindo uma formatação limpa durante a edição do conteúdo. (SITES-27780)
...
Adiciona atributos ausentes do `title` aos botões da caixa de diálogo da interface do usuário Coral, como **Concluído** e **Cancelar** nas caixas de diálogo de edição de componente para melhorar a acessibilidade e habilitar a validação automatizada. Garante que os botões mantenham os atributos esperados na renderização de marcação, evitando falhas nos testes de interface baseada no Selenium. (NPR-42412)

#### WCM{#foundation-wcm-6523}

Correção de um problema que impedia que as páginas fossem adicionadas a trabalhos de tradução ao usar a **Cópia de Idioma de Atualização** em ambientes com o Service Pack 19 ou posterior. Garante que os fluxos de trabalho de tradução prossigam conforme o esperado, permitindo a transferência adequada de páginas entre cópias de idioma sem intervenção manual. (CQ-4357929)
...
AEM 6.5.21, 6.5.22, 6.5.23 e AEM 6.5 LTS GA são fornecidos com o pacote `org.apache.sling.scripting.jsp:2.6.0`, que contém um problema conhecido. O problema normalmente ocorre com cargas altas, quando a instância do AEM trata muitas solicitações simultâneas.
  ...
  Entre em contato com o Suporte ao cliente da Adobe e consulte esta nota de versão para obter uma solução.

* **Relacionado ao Carvalho**
No Service Pack 13 e posterior, o seguinte registro de erro começou a aparecer, afetando o cache de persistência:
...
* Se você atualizar a instância do [!DNL Experience Manager] de 6.5.0 para 6.5.4 para o Service Pack mais recente no Java™ 11, verá `RRD4JReporter` exceções no arquivo `error.log`. Para interromper as exceções, reinicie a instância do [!DNL Experience Manager]. <!-- THIS BULLET POINT WAS UPDATED AS PER CQDOC-20021, JANUARY 23, 2023 -->
...
* **FORMS-16557**: na Visualização de Impressão da Interface do Usuário do Agente de Comunicações Interativas, o símbolo de moeda (como cifrão $) é exibido de forma inconsistente para todos os valores do campo. Aparece para valores até 999, mas está ausente para valores de 1000 e superiores.
...
* **FORMS-16603**: na Visualização de Impressão da interface do usuário do Interative Communications Agent, alguns valores calculados não são exibidos corretamente.
* **FORMS-15681**:When a letra é exibida na Visualização de Impressão, o conteúdo é alterado. Ou seja, alguns espaços desaparecem e certas letras são substituídas por `x`.
* **FORMS-15428**: após atualizar para o AEM Forms Service Pack 20 (6.5.20.0) com o Complemento do Forms, as configurações que dependem do Serviço Adobe Analytics Cloud herdado usando a autenticação baseada em credencial param de funcionar. Esse problema impediu que as regras de análise fossem executadas corretamente.
...
Esses sites estão disponíveis apenas para os clientes. Se você for cliente e precisar de acesso, entre em contato com o seu gerente de conta da Adobe.
