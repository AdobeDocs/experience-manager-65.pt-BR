---
title: Notas de versão do service pack '[!DNL Adobe Experience Manager] 6.5.3'
...
* Os nomes dos ativos são alterados para letras minúsculas ao mover ativos de uma pasta para outra no [!DNL Experience Manager] em execução no modo [!DNL Dynamic Media]-Scene7 (NPR-31630).
...
* Enormes trabalhos do sling são criados e o Banner de processamento aparece novamente quando ações feitas em ativos no [!DNL Experience manager Assets] são passadas para o Scene7 (NPR-30947).
...
* O fluxo de trabalho Codificação de Vídeo do [!DNL Dynamic Media] não está conseguindo gerar a miniatura do vídeo migrado do modo [!DNL Dynamic Media Classic] para o modo [!DNL Dynamic Media]-Scene7 no Adobe Experience Manager (CQ-4282011).
...
* O uso do botão Mover para mover vários ativos, de uma pasta para outra, falha no [!DNL Experience Manager] em execução no modo de execução do [!DNL Dynamic Media]-Scene7 (CQ-4280384).
...
* Os links rápidos para gerenciar grupos (Abrir/Editar/Publish/Excluir grupos) não estão visíveis para os administradores da Comunidade (Administrador do grupo/Administrador do site) (NPR-31627).
...
* O serviço Web SOAP usando MTOM em um projeto .NET exibe exceções para invocar AssemblerServiceClient e métodos HtmlToPDF2 (NPR-4281771).

* Vulnerabilidade de segurança 2012-5784 e 2014-3596 encontradas com o jar AXIS 1.4 e correção fornecida com o [AXIS1.4.1 jar](https://helpx.adobe.com/aem-forms/quick-fixes/6-5/jee-patch-0014.html) (NPR-31015).
...
O UberJar do Experience Manager 6.5.3.0 está disponível no [repositório central Maven](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.3/).
