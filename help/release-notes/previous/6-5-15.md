...
* Quando o usuário aplica o AEM Forms Service Pack 14 (SP14) do AEM Forms implantado usando o IBM® WebSphere®, o bootstrapping falha ao inicializar um banco de dados e ocorre o erro `java.lang.NoClassDefFoundError:org/apache/log4j/Logger`.(NPR-39414)
...
* O GraphQL gera uma exceção. Por exemplo, não é possível obter tags de variação de um fragmento de conteúdo. There is no variation with the name &#39;electric&#39;. This issue is due to calling `getVariationTags` for a non-existing variation which raises an exception. (SITES-8898)
