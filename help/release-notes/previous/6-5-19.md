...
* **[Aprimoramentos de acessibilidade no AEM Forms Designer](https://helpx.adobe.com/content/dam/help/pt-br/experience-manager/6-5/forms/pdf/using-designer.pdf)**: o AEM Forms Designer apresenta as seguintes melhorias de acessibilidade:

   * O AEM Forms Designer agora oferece suporte a Sumários acessíveis por meio das novas marcas `<TOC>` e `<TOCI>`, permitindo que os autores atribuam essas funções aos subformulários por meio da guia Acessibilidade. O recurso é compatível com estruturas de sumário de vários níveis, entradas rotuladas, referências numeradas e subestruturas de extensão, permitindo que os leitores de tela identifiquem e naveguem corretamente no conteúdo do sumário.

   * O AEM Forms Designer agora é compatível com a configuração de propriedades de acessibilidade para células de tabela, incluindo Escopo, Expansão de linha e Expansão de coluna, permitindo que os leitores de tela e dispositivos de assistência interpretem corretamente a estrutura da tabela em PDFs gerados. Os autores podem definir essas propriedades diretamente na caixa de diálogo Propriedades da célula e verificá-las na visualização XML Source. As propriedades configuradas são refletidas com precisão na saída do PDF marcada e podem ser validadas usando o Acrobat Table Editor.
...
* A ordenação das tags é alterada aleatoriamente no PDF. A marcação no Forms Designer está correta. (LC-3921313) 
...
1. O pacote OSGi `org.apache.jackrabbit.oak-core` é versão 1.22.17 ou posterior (Use o Console da Web: `/system/console/bundles`).
...
Para resolver o problema, [recompile os scripts Sling, como JSP, Java™ e Sightly](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-16543.html?lang=pt-BR#resolution). (FORMS-8542)
