...
* Quando você ativa o SSL para uma instância do Experience Manager, o caminho do contexto é removido (NPR-35477).
...
* Ao replicar um ativo do AEM que inclui um colchete (). no nome, a replicação falha (GRANITE-27004, NPR-35315).
...
* Quando várias páginas acessam simultaneamente uma biblioteca do cliente, o gerenciador de bibliotecas do HTML não carrega a biblioteca do cliente. Ele resulta na renderização incorreta de páginas (NPR-35538).
...
* [!DNL Assets] não exibe a representação do RGB para um ativo que tenha representação do RGB (CQ-4310190).
...
* Para ativos carregados que são publicados automaticamente no DM, o status não exibe Publicado no AEM. Além disso, a coluna de status Publicação do Dynamic Media não mostra o status publicado correto (CQ-4306415).
...
* A guia [!UICONTROL Tarefa] do espaço de trabalho do HTML exibe caracteres HTML (NPR-35298).
...
* O PDF forms gerado usando o Serviço de saída não inclui bordas (CQ-4312564).
...
### Outros aprimoramentos (6.5.8.0) {#other-enhancements-forms-6580}
...
* Se você estiver atualizando a instância do [!DNL Experience Manager] da versão 6.5 para a versão 6.5.8.0, poderá exibir `RRD4JReporter` exceções no arquivo `error.log`. Reinicie a instância para resolver o problema.
* Entre em contato com o Suporte ao Cliente da Adobe se você encontrar problemas ao editar e criar regras em cascata no [!UICONTROL Editor do Forms de Esquemas de Metadados de Pastas] e no [!UICONTROL Editor do Forms de Esquemas de Metadados] usando a caixa de diálogo [!UICONTROL Definir Regra]. As regras que já foram criadas e salvas estão funcionando como esperado.
