...
* Acessibilidade aprimorada em várias áreas no Gerenciamento de conteúdo da Web (WCM) do Experience Manager, como Editor de páginas, Componentes principais, RTE e Interface do usuário do administrador.
...
* Ao criar um componente personalizado usando o componente Fragmento de experiência padrão e usá-lo nas páginas do Experience Manager Sites, o Experience Manager não exibe referências (uso) para o componente personalizado (NPR-32852).
...
* Usuários anônimos podem acessar os recursos do CRXDE Lite (GRANITE-27790).
...
>[!IMPORTANT]
>
>Os usuários do Windows de [!DNL Experience Manager desktop app] são aconselhados a atualizar para a [versão do aplicativo de desktop 2.0.3.2](https://experienceleague.adobe.com/docs/experience-manager-desktop-app/using/release-notes.html?lang=pt-BR#what-is-new) para acessar o repositório DAM na instância [!DNL Adobe Experience Manager 6.5.5.0]. Como podem encontrar problemas ao acessar o repositório DAM na instância [!DNL Adobe Experience Manager] 6.5.5.0 usando o aplicativo de desktop versão 2.0.2.
...
[!DNL Adobe Experience Manager] 6.5.5.0 O Assets fornece correções para os seguintes problemas:
...
* Durante o processo de write-back do XMP, todas as propriedades de metadados de namespace personalizadas alteram o prefixo do namespace personalizado para ns2 em vez do prefixo do namespace configurado (NPR-32748).
...
* A criação de uma configuração do Dynamic Media no Experience Manager funciona, mas na interface do usuário nada acontece ao selecionar Salvar (CQ-4292442).
...
* Problemas de desempenho corrigidos em 6.5.5.0 são (CQ-4279206):
   ...
   * Demora muito para carregar binários grandes em servidores de Processamento de imagem do Dynamic Media.
...
* Publicar e exibir ativos 3D comuns em [!DNL Experience Manager Sites] páginas usando o componente WCM [!DNL Sites].
...
* O Experience Manager trava devido à falha de segmentação com [!DNL Apache Lucene] (NPR-32988).
* Pacote principal [!DNL Jackson] ausente do arquivo uberjar do Experience Manager (NPR-32848).
...
* O agendador de tarefas de manutenção [!DNL Granite] reinicializa com muita frequência durante implantações Experience Manager (CQ-4294627).
...
* Os usuários do IMS autorizados no grupo de administradores local do Experience Manager não podem criar ou modificar as configurações do IMS (NPR-33045).
...
* Tags traduzidas não são importadas para o Experience Manager de serviços de tradução de terceiros (NPR-33154).
...
* Acessibilidade adaptável do Forms: quando um usuário pré-visualiza um formulário adaptável como um formulário do HTML, o campo [!UICONTROL Assinatura Escrita] não consegue manter o foco da guia (NPR-33159).
...
* Serviço PDFG: falha na operação `CreatePDF` ao converter um arquivo PDF para o formato OCR do PDF (NPR-32994).
* Serviço PDFG: falha na conversão do PDF para a 200ª instância de um documento [!DNL OpenOffice] (NPR-32766).
...
* Designer: se a opção de marcação estiver ativada, a borda do subformulário desaparecerá na saída do PDF gerada (NPR-32778).
...
* O modelo de dados de formulário agora oferece suporte à autenticação baseada em certificado X-509 ao usar os serviços Web da SOAP como fonte de dados. Para obter mais informações, consulte [Configurar serviços Web do SOAP](/help/forms/using/configure-data-sources.md#configure-soap-web-services).
