...
* Publique ou cancele a publicação seletiva de ativos em [!DNL Experience Manager] ou [!DNL Dynamic Media] usando o assistente de [!UICONTROL Publicação rápida] ou [!UICONTROL Gerenciar Publicação].
...
* As opções padrão para `test` e `aiprocess` são atualizadas para `Thumbnail`, de `Rasterize` anteriormente no Dynamic Media, para garantir que os usuários precisem criar somente a miniatura e ignorar a extração de página e a extração de palavra-chave.
...
* Suporte para [Marcas de texto do Adobe Sign no Serviço de conversão automática de formulários](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/convert-existing-forms-to-adaptive-forms.html?lang=pt-BR).
...
* Em [!DNL Sites] ou [!DNL Screens], selecione um Projeto e clique em [!UICONTROL Publicações de Gerenciamento]. Os usuários não podem avançar no assistente [!UICONTROL Gerenciar Publicação] devido a erros de interface. A opção [!UICONTROL Publicar] não funciona especificamente (NPR-34099).
...
* No componente principal de Texto, após usar a opção de edição de origem do HTML, a classe da tag `em` é removida (NPR-34081).
...
* A operação [!UICONTROL Gerenciar Publicação] para replicar o conteúdo da instância Autor para Publicação falha e gera um erro de JavaScript (NPR-33872).
...
* Você pode selecionar [!UICONTROL HTML e JSON] como opção para [!UICONTROL formato de exportação do Adobe Target] em uma pasta pai na seção [!DNL Experience Fragments]. As mesmas propriedades são exibidas na interface habilitada para toque para as subpastas dessa pasta principal. No entanto, no CRXDE, para `cq:adobeTargetExportFormat`, ele exibe apenas o HTML em vez de exibir `html,json` (NPR-33423).
...
* Uma página da Web com o caractere `/` na URL não responde durante a criação. Quando um componente é adicionado durante a criação, o uso do CPU aumenta e o navegador para de responder (CQ-4295749).
...
* Criação de script entre sites na Visualização de calendário da Caixa de entrada do Experience Manager (NPR-33545).
* Uma URL que termina com `childrenlist.html` exibe uma página do HTML em vez de uma resposta 404. Esses URLs são vulneráveis a script entre sites (NPR-33441).
...
* A página [!UICONTROL Mover Assets] para ativos em coleções não carrega todo o conteúdo do HTML, como a opção [!UICONTROL Ajustar/Republicar]. Portanto, os usuários não podem concluir a operação de movimentação (NPR-33860).
...
* O fluxo de trabalho de ativos de atualização do DAM é acionado quando o usuário rola para baixo na página [!UICONTROL Arquivo de fluxo de trabalho], na guia [!UICONTROL Fluxo de trabalho], em [!UICONTROL Ferramentas] no [!DNL Adobe Experience Manager] configurado com o Dynamic Media Scene7 (CQ-4299727).
...
* As alterações nas predefinições de imagem no [!UICONTROL Adobe Experience Manager] não são sincronizadas com o Sistema de Publicação do Scene7 (CQ-4299713).
...
* Não é possível baixar logs usando a ferramenta de Diagnóstico em uma instância do Experience Manager atualizada (NPR-34336).
...
Após instalar o pacote complementar [!DNL Experience Manager Forms] 6.5.6.0:
...
* A descrição do conteúdo da ajuda de campos de formulário adaptáveis exibe uma tag de parágrafo HTML (NPR-34116).
...
* Ao instalar o AEM Service Pack, a lista de itens **Tarefas Pendentes** não é exibida como links. O texto dos itens **Tarefas Pendentes** inclui as marcas HTML (NPR-34317).
...
* Ao gerar um arquivo PDF a partir de um arquivo XDP usando os serviços de saída e Forms, o resultado é um texto ausente e sobreposto (NPR-34237, CQ-4299331).

* Quando você converte um arquivo HTML em PDF, o atributo `MaxReuseCount` não é configurável (NPR-33470).

* Ao baixar um arquivo do PDF que inclui recursos interativos do Reader Extensions, você não pode adicionar um anexo ao arquivo do PDF usando o [!DNL Adobe Reader] (NPR-33729).
...
### Cache aprimorado para páginas de formulário adaptável traduzidas (6.5.6.0) {#enhanced-caching-translated-adaptive-forms}
