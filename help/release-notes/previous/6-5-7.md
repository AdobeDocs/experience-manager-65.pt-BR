...
* O [!DNL Experience Manager] 6.5 Service Pack 7 Forms melhora o desempenho do:
   ...
   * Convertendo um formulário do PDF em um formulário adaptável usando o [!DNL Automated Forms Conversion service].
...
* Quando você atualiza para [!DNL Experience Manager] 6.5.6.0, o comportamento do sistema de parágrafos herdado é alterado e não funciona corretamente (NPR-35117).
...
* Usuários sem privilégio de administrador podem editar ativos expirados usando o aplicativo de desktop [Experience Manager]. (NPR-34993).
...
* O vídeo [!DNL Dynamic Media] não está sendo reproduzido em dispositivos iOS com os controles de vídeo nativos do HTML5 (CQ-4306977, CQ-4306727).
...
* Desfazer a publicação de um arquivo de vídeo de [!DNL Experience Manager] não cancele a publicação do Conjunto de vídeos adaptados de uma implantação do Scene7 configurada (CQ-4304405).
...
* O HTML5 eCatalog Viewer em dispositivos móveis iPhones não pode virar as páginas ou virar as páginas (CQ-4296611).
...
O UberJar para Experience Manager 6.5.7.0 está disponível no [repositório central Maven](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.7/).
