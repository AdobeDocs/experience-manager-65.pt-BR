---
title: Notas de versão para  [!DNL Adobe Experience Manager] 6.5.13.0
...
* Quando você localiza um Formulário adaptável e faz até mesmo uma pequena alteração no texto do idioma base, a tradução completa fica ausente para todos os outros idiomas. O problema foi corrigido em [!DNL Experience Manager] 6.5.13.0. (NPR-37189)
...
* Ao criar uma Predefinição de imagem para Dynamic Media no [!DNL Experience Manager], o máximo permitido é limitado a 2000x2000 pixels na interface. Quando o valor é aumentado para 2001 pixels de largura ou altura, o botão **[!UICONTROL Salvar]** é desativado. (ASSETS-5691)
...
* Os usuários são desconectados aleatoriamente das [!DNL Experience Manager] instâncias de publicação e um Formulário adaptável não é enviado. O problema aparece em [!DNL Experience Manager] instâncias que usam o Dispatcher. (NPR-38285)
* Ao usar a opção getFormDataString em uma regra do Adobe Launch para capturar os dados do Formulário adaptável, a opção não retorna os dados do Forms adaptável. (NPR-38283)
...
* Ao usar com.adobe.fd.pdfutility.services.PDFUtilityService#convertPDFtoXDP para converter um formato de PDF em XDP, um arquivo XDP inválido será retornado. (NPR-38140, CQ-4342099)
...
* O componente AEM Forms incorporado na página SITES usa o atributo width que tem valor em % e não é válido de acordo com a validação W3C HTML. Os usuários encontram um erro de análise inválido durante a validação do HTML. (NPR-38124)
...
* Quando um usuário adiciona tags do HTML à seção de comentário ao executar um fluxo de trabalho, as tags do HTML são renderizadas. (NPR-37591)
...
* A partir da versão 6.5.13, não será mais necessário criar o nó do componente de contêiner no JCR antes de fazer edições por meio do Editor de SPA. Um `virtual container` é criado antes de ser salvo por meio da SDK. (SITES-5762)
...
* Atualize para `favicon.ico` que é usado no Experience Manager. (CQ-4315324)
...
>[!NOTE]
>
>A Adobe não recomenda remover ou desinstalar o pacote [!DNL Experience Manager] 6.5.13.0.
...
>[!NOTE]
>
>[!DNL Experience Manager] 6.5.13.0 não dá suporte à instalação do Bootstrap.
...
* [Fale com o suporte ao cliente da Adobe](https://experienceleague.adobe.com/docs/customer-one/using/home.html?lang=pt-BR).
