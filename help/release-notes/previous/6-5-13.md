...
O [!DNL Experience Manager] 6.5.13.0 inclui novos recursos, importantes melhorias solicitadas por clientes e melhorias de desempenho, estabilidade e segurança, lançadas desde a disponibilização inicial do 6.5 em abril de 2019. [Instalar este service pack](#install) em [!DNL Experience Manager] 6.5.
...
* Depois de atualizar para o [!DNL Experience Manager] 6.5 SP10, o editor de metadados de ativos não está funcionando corretamente. (CQ-4341359)
* Ao abrir uma coleção inteligente que é salva com o filtro de pesquisa aplicado como Publish, o filtro de pesquisa muda automaticamente para Não publicado. (CQ-4341191)
* Ao alternar o idioma na **[!UICONTROL Preferência do Usuário]**, o rótulo **[!UICONTROL Classificar por]**, o botão suspenso e outras opções nas opções de classificação da página inicial do Ativo não serão refletidos no idioma selecionado. (CQ-4339306)
* Ao adicionar uma regra a um campo suspenso em **[!UICONTROL Esquema de Metadados]**, a lista **[!UICONTROL Dependente de]** não reflete o rótulo do campo suspenso. (ASSETS-9442)
* O menu suspenso Assets Metadata desabilitado não retém valores. (ASSETS-8918)
* Ao exibir o ativo usando a opção **[!UICONTROL Mais Detalhes]** na exibição **[!UICONTROL Coluna]**, anotações incorretas são exibidas. (ASSETS-8851)
...
* Ao criar uma Predefinição de imagem para o Dynamic Media em [!DNL Experience Manager], o máximo permitido é limitado a 2000x2000 pixels na interface. Quando o valor é aumentado para 2001 pixels de largura ou altura, o botão **[!UICONTROL Salvar]** é desativado. (ASSETS-5691)
...
* Acessibilidade - Os leitores de tela não estão narrando a função de cabeçalho para o texto `Cannot save this set` no alerta `Cannot save this set`, ao navegar usando as teclas de atalho de cabeçalho `H` e Seta para Baixo. (ASSETS-5569)
...
* &quot;everyone&quot; tem jcr:write em `/content/usergenerated/etc/commerce/smartlists`. (NPR-35230)
* A classificação local de produtos Commerce não funciona mais. (CQ-4343750)
* Não é possível executar o Publish rápido em um produto da página de resultados da pesquisa após alterar as propriedades. (CQ-4343318)
...
* A codificação de código de barras PDF417 produzida pelo Forms [!DNL Experience Manager] é inválida para um grupo de botões de opção. (NPR-38525)
...
* Os usuários experimentam texto sobreposto no Adaptive Forms após aplicar o [!DNL Experience Manager] 6.5 Service Pack 11. (NPR-38376, CQ-4342472)
...
* Quando você usa um Fluxo de Trabalho [!DNL Experience Manager] para enviar um email em hebraico, o email recebido no final do usuário contém pontos de interrogação (??) em vez de texto em hebraico (NPR-38296).
* Os usuários são desconectados aleatoriamente de [!DNL Experience Manager] instâncias do Publish e um Formulário adaptável não é enviado. O problema aparece em [!DNL Experience Manager] instâncias que usam o Dispatcher. (NPR-38285)
...
* Ao chamar um método de serviço de ponto de extremidade SOAP do serviço do AEM Forms JEE em um clique de botão configurado como uma regra de Formulário adaptável, o serviço SOAP falha, com a exceção abaixo:
...
* Ao importar e publicar uma correspondência que inclua um novo arquivo XDP, elas não são visualizadas na instância do Publish. No entanto, se as cartas forem importadas e publicadas uma segunda vez usando o mesmo arquivo CMP, elas serão visualizadas com êxito. (CQ-4343599)
...
* Não é possível importar um arquivo do Microsoft® Word para o Forms Designer. O usuário encontra o erro `Word (version XP or onwards) could not be found on the machine`. (CQ-4342146)
* O usuário não pode definir um horário personalizado no Agendador de limpeza de trabalhos. (CQ-4339192)
* O usuário não pode atualizar nenhuma configuração na interface de gerenciamento de ponto de extremidade e encontrou o erro ` Uncaught ReferenceError: updateEndpoint_required is not defined`. (CQ-4331523)
...
* Melhore o tempo de resposta ao abrir o Editor de modelos de fragmentos de conteúdo. Clientes com vários fragmentos no Assets podem ter visto erros ao abrir o. (SITES-6284)
...
* Usar **[!UICONTROL Editar]** dos resultados da pesquisa pode resultar em um erro `Not Found`. (NPR-37810)
...
* Habilitar suporte para a próxima versão dos Componentes Principais de Email [https://github.com/adobe/aem-core-email-components](https://github.com/adobe/aem-core-email-components). (NPR-38445 e NPR-38204)
...
* O menu suspenso Sistema de estilos pode ter sido posicionado na parte superior da página em vez de no contexto do componente - para componentes que usam `cq:editConfig` &quot;afteredit: REFRESH_PAGE&quot;. Esse problema foi resolvido. (NPR-38384)
...
* A partir da versão 6.5.13, não será mais necessário criar o nó do componente de contêiner no JCR antes de fazer edições por meio do Editor de SPA. Um `virtual container` é criado antes de ser salvo por meio do SDK. (SITES-5762)
...
* Atualização do pacote `sling-javax.activation` com a correção de SLING-8777. (NPR-38077)
...
* Alteração do comportamento de adicionar páginas a projetos de tradução desde o Service Pack 10. O projeto de tradução não contém uma página recém-criada [ex: test-page-women-2] na lista, quando selecionada, a página pai da página recém-criada [não é diretamente a página recém-criada]. (NPR-38256)
* Adicionar a propriedade `cq:isTranslationLaunch` em Inicializações criadas por Projeto de Tradução. (NPR-38224)
* O Launch está sendo criado para uma página que tem um XF referenciado que tem um ativo. (NPR-38199)
* A atualização da memória de tradução [!DNL Experience Manager] não funciona. (NPR-38196)
* Habilitar ES6 para `/libs/cq/gui/components/projects/admin/translation/job/addcontent/clientlibs.js`. (NPR-38306)
* Último pacote de 18n para traduções para [!DNL Experience Manager] 6.5. (CQ-4339505)

## Interface do usuário {#ui-6513}

* Atualização para `favicon.ico` que é usada no Experience Manager. (CQ-4315324)
* Quando você estiver na página inicial > seção Ferramentas e clicar no ícone [!DNL Experience Manager], a tela Navegação [!DNL Experience Manager] deverá aparecer. (NPR-38417)
...
## Instalar o [!DNL Experience Manager] 6.5.13.0 {#install}

<!-- Remaining content from here to bottom stays the same except for version updating as needed as per update team feedback -->

* [!DNL Experience Manager] 6.5.13.0 requer [!DNL Experience Manager] 6.5. Consulte a [documentação de atualização](/help/sites-deploying/upgrade.md) para obter instruções detalhadas.
...
* Em uma implantação com MongoDB e várias instâncias, instale o [!DNL Experience Manager] 6.5.13.0 em uma das instâncias do Autor usando o Gerenciador de Pacotes.

>[!NOTE]
>
>O Adobe não recomenda remover ou desinstalar o pacote [!DNL Experience Manager] 6.5.13.0.
...
>[!NOTE]
>
>O [!DNL Experience Manager] 6.5.13.0 não oferece suporte à instalação do Bootstrap.
...
1. A página de informações do produto (`/system/console/productinfo`) exibe a cadeia de caracteres da versão atualizada `Adobe Experience Manager (6.5.13.0)` em [!UICONTROL Produtos Instalados].
...
1. O pacote OSGi `org.apache.jackrabbit.oak-core` é versão 1.22.11 ou posterior (Use o Console da Web: `/system/console/bundles`).
...
>[!NOTE]
>
>Ignore se você não estiver usando o Forms [!DNL Experience Manager]. As correções no Forms [!DNL Experience Manager] são entregues por meio de um pacote complementar separado uma semana após a versão programada do Service Pack [!DNL Experience Manager].
...
O UberJar do [!DNL Experience Manager] 6.5.13.0 está disponível no [repositório central do Maven](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.13/)(https://).
