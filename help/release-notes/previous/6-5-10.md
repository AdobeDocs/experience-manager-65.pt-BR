...
* **Suporte ao Camera Raw**: um novo pacote [!DNL Camera Raw] está disponível e oferece suporte ao [!DNL Adobe Camera Raw] v10.4. Consulte [processar imagens usando [!DNL Camera Raw]](/help/assets/camera-raw.md).
...
* Agora você pode usar o serviço Conversão automática de formulários para [converter o PDF forms em francês, alemão, espanhol, italiano e português](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/extending-the-default-meta-model.html#language-specific-meta-model) para formulários adaptáveis.
...
* **Suporte para usar a opção literal para definir valor para uma variável do tipo JSON**: você pode usar a opção literal para definir valor para uma variável do tipo JSON na etapa Definir Variável de um Fluxo de Trabalho do AEM. A opção literal permite especificar um JSON no formato de uma string.
...
* Depois de atualizar para o service pack 9, alguns usuários não podem mover pastas no Experience Manager e ver erros nos logs (SITES-1481).
...
* As páginas da Web não são renderizadas quando os links são modificados por pacotes de filtros de servlet personalizados e assim por diante. (NPR-36857) <!-- Note updated as per CQDOC-19203, April 07, 2022 -->
...
* Quando há uma estrutura de site com muitas subpáginas, o carregamento das subpáginas na exibição de coluna leva mais tempo no Experience Manager 6.5.8 do que no Experience Manager 6.4.8.2 (CQ-4322766).
...
* O valor da propriedade `jcr:title` não é atualizado na instância Publish depois que uma pasta é movida. Renomear e republicar uma pasta no autor não atualiza o valor da propriedade `jcr:title` do mesmo na instância de Publicação (NPR-36369).
...
* Os leitores de tela agora narram o nome, a função e o estado padrão (selecionado ou não) das opções nas opções [!UICONTROL Todas as Assets] e [!UICONTROL Todas as coleções] ao navegar pela página [!UICONTROL Publicar] (CQ-4290705).
...
* Alguns nomes de texto e controle são mais fáceis de ler para usuários com problemas de visão, pois a taxa de luminosidade atende à taxa mínima necessária de 4,5:1 (NPR-36503).
...
* O Experience Manager deixa de responder quando o nó primário do MongoDB muda para outro nó (NPR-36566).
...
* Atraso na exibição de caracteres na guia [!UICONTROL Dados] e na visualização de carta do HTML (NPR-37020).
...
* Ao editar um fragmento de documento de texto e, em seguida, visualizar a correspondência, o AEM Forms exibe o idioma da expressão na visualização de correspondência do HTML (CQ-4322331).
...
* O Screen Reader não lê os dados de campo flutuante colocados no rótulo de texto na página principal ou nas páginas de Subformulário em um PDF dinâmico (CQ-4321587).

**Serviços de documento**

* Quando você converte arquivos XDP em arquivos PDF e, em seguida, monta o PDF resultante, as gerações do PDF falham e exibem a seguinte mensagem de erro:
...
* Ao renderizar um XDP como HTML usando `dataXml`, o [!DNL AEM Forms] exibe um erro `Page Unresponsive` (NPR-36631).
...
* Os fluxos de trabalho **[!UICONTROL Publicar mais tarde]** e **[!UICONTROL Cancelar publicação mais tarde]** não funcionam (CQ-4327679).
...
* Se você estiver atualizando a instância do [!DNL Experience Manager] da versão 6.5 para a versão 6.5.10.0, poderá exibir `RRD4JReporter` exceções no arquivo `error.log`. Para resolver o problema, reinicie a instância.
