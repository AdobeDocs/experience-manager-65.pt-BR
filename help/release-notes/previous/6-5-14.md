...
* Falha no processamento de PDF com uma mensagem de erro `NoClassDefFoundError`. (NPR-38741)
...
* Na exibição de Cartão, na área de navegação do arquivo principal, o conteúdo do texto **[!UICONTROL Classificar por]** não tem pelo menos uma relação de contraste de 4.5:1 em relação à cor do plano de fundo. (ASSETS-6926)
* O Experience Manager não identifica **[!UICONTROL Selecione uma lista suspensa de modelo de fluxo de trabalho]** como um campo obrigatório ao criar um modelo de fluxo de trabalho. (ASSETS-6871)
...
* O Assets não está sendo carregado para a conta do Dynamic Media devido à ausência do parâmetro de upload. (ASSETS-13165)
...
* As **[!UICONTROL Configurações Gerais]** e a **[!UICONTROL Instalação do Publish]** da Dynamic Media não devem estar acessíveis para usuários não administradores. (ASSETS-8628)
...
* O Assets não é carregado na conta do Dynamic Media depois de executar o fluxo de trabalho Reprocessamento do Dynamic Media Assets em ativos com identificador de trabalho incorreto. (ASSETS-12084, ASSETS-9877)
...
* O leitor de tela não lê os botões **[!UICONTROL Ativar mudo]** e **[!UICONTROL Ativar mudo]** em vídeo usando o componente Dynamic Media. (ASSETS-10169)
...
* Para PDF forms estáticos criados com o Designer do Forms 6.5, a acessibilidade do PDF falha com o erro `Tab order entry in page with annotations not set to "S"`. (CQ-4343117)
...
* Ao usar **[!UICONTROL Selecionar Tudo]** > **[!UICONTROL Publish Rápido]**, o Experience Manager não estava publicando todos os ativos ou mostrando quantos ativos seriam publicados no modo de exibição **[!UICONTROL Cartão]** ou **[!UICONTROL Lista]**. (NPR-38546)
...
## Instalar o [!DNL Experience Manager] 6.5.14.0 {#install}
...
* Em uma implantação com MongoDB e várias instâncias, instale o [!DNL Experience Manager] 6.5.14.0 em uma das instâncias do Autor usando o Gerenciador de Pacotes.<!-- UPDATE FOR EACH NEW RELEASE -->

>[!IMPORTANT]
>
>O Adobe não recomenda que você remova ou desinstale o pacote [!DNL Experience Manager] 6.5.14.0. Sendo assim, antes de instalar o service pack, você deve criar um backup do `crx-repository`, caso precise revertê-lo. <!-- UPDATE FOR EACH NEW RELEASE -->
...
O UberJar do [!DNL Experience Manager] 6.5.13.0 está disponível no [repositório central do Maven](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.13/). <!-- CHECK FOR UPDATE EACH NEW RELEASE -->
