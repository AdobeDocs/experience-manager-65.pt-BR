...
| Data | sexta-feira, 24 de agosto de 2023 <!-- UPDATE FOR EACH NEW RELEASE --> |
...
* **[Enhanced error handling with custom error handlers in the rule editor](https://experienceleague.adobe.com/docs/experience-manager-65/forms/adaptive-forms-advanced-authoring/standard-validation-error-messages-adaptive-forms.html?lang=pt-BR)** – You can now invoke a custom function (using Client Library) in response to an error returned by an external service. And, you can provide a tailored response to end users. Como alternativa, você pode realizar ações específicas para erros retornados por um serviço. For example, you can invoke a custom workflow in the backend for specific error codes or inform the customer that the service is down

* **[Enhanced Adobe Sign Workflow step](https://experienceleague.adobe.com/docs/experience-manager-65/forms/workflows/aem-forms-workflow-step-reference.html?lang=pt-BR#sign-document-step)** – Adobe Sign workflow step in AEM Workflows is available with the following enhancements.
...
* A implementação do GraphQL do Experience Manager 6.5 não estava de acordo com o principal e várias correções importantes estavam ausentes. (SITES-13096)
...
* Os estilos adicionados ao Experience Manager 6.5, Service Pack 17, para âncora e marca de corpo estavam causando problemas de CSS. As tags de âncora não eram exibidas sublinhadas no Autor. (SITES-14261)
...
* O Experience Manager exibe mensagens de erro e aviso ao carregar um ativo na instância 6.5.17.0 do AEM. (ASSETS-26232)
...
* Corrigido o download da representação de mídia dinâmica de marca d&#39;água. (ASSETS-26173)
...
* **Serviços de documento**
   ...
   * Se o servidor for desligado durante a geração de documentos do PDF, erros de processamento de tarefas de inicialização pós-servidor serão gerados. O argumento -Dcom.adobe.livecycle.dsc.deferServiceStart=true deve ser adicionado durante a inicialização do servidor. (FORMS-9836)
   * Se um usuário tentar mesclar PDFs usando o método AssemblerService.Invoke, o montador não executará a tarefa. (FORMS-9550)
   * Quando você atualiza para o AEM 6.5.15.0 Service Pack em ambientes OSGI e JEE, o serviço do Assembler que usa um modelo específico para de funcionar. (FORMS-9355, FORMS-9445, FORMS-9408)
   ...
   * Ao renderizar a pré-visualização de PDF de um Formulário adaptável, os despejos de pilha indesejados do Java™ são exibidos nos logs de erro. (FORMS-8865)
   ...
   * Depois da atualização para o AEM Service Pack 6.5.14.0, o problema surge no serviço de assembler ao usar um modelo específico. (FORMS-9445, FORMS-9408)
...
* **Formulários adaptáveis**
   ...
   * Se um Formulário Adaptável for recarregado depois de atualizar para o AEM 6.5.16.0 Service Pack, a recuperação do anexo de arquivo falhará. (FORMS-8906)
...
* Após uma instalação básica do Experience Manager, Service Pack 17, você vê erros no `stderr.log`. As instalações do Vanilla não devem receber erros. (CQ-4353637)
* O botão Create na tela Tagging não respeita a ACL (Access Control List, Lista de controle de acesso). (NPR-40973)
* Não é possível criar ou acessar o nó de cache do ContextHub no Experience Manager, ou ambos. (NPR-40515)
...
* Com o AEM 6.5.15.0, todas as URLs personalizadas, aliases de sling e mapeamento de sling são interrompidos após uma reinicialização do AEM. (NPR-40420)
...
* O download do service pack está disponível na [Distribuição de software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/servicepack/aem-service-pkg-6.5.18.0.zip) da Adobe.
...
>[!IMPORTANT]
>
> A Adobe não recomenda que você remova ou desinstale o pacote [!DNL Experience Manager] 6.5.18.0. Dessa forma, antes de instalar o pacote, você deve criar um backup do `crx-repository`, caso precise revertê-lo. <!-- UPDATE FOR EACH NEW RELEASE -->
...
>[!NOTE]
>
>O Experience Manager 6.5.18.0 não oferece suporte à instalação do Bootstrap. <!-- UPDATE FOR EACH NEW RELEASE -->
...
1. O pacote OSGi `org.apache.jackrabbit.oak-core` é versão 1.22.16 ou posterior (Use o Console da Web: `/system/console/bundles`).
...
O UberJar para [!DNL Experience Manager] 6.5.18.0 está disponível no [repositório central Maven](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.18/). <!-- CHECK FOR UPDATE EACH NEW RELEASE -->
...
* **A publicação de páginas não está funcionando no Editor de Páginas após a atualização para o Service Pack 18 (6.5.18.0)**
  ...
  Depois de atualizar uma instância do AEM 6.5.0.0—6.5.17.0 para o AEM 6.5.18.0, ao clicar em **Publicar Página** dentro do Editor de Páginas, você será redirecionado para uma URL que não existe.
