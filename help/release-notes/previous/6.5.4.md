---
title: '[!DNL Adobe Experience Manager] Notas de versão do 6.5.4 service pack'
description: Notas de versão do [!DNL Adobe Experience Manager] 6.5.4 service pack
contentOwner: AK
mini-toc-levels: 2
...
O Adobe Experience Manager 6.5.4.0 é uma atualização importante que inclui novos recursos, principais melhorias solicitadas por clientes e desempenho, estabilidade e aprimoramentos de segurança, lançados desde a disponibilização geral da versão 6.5 do **Abril de 2019**. Ele pode ser instalado sobre o Adobe Experience Manager 6.5.

Alguns recursos e aprimoramentos principais introduzidos no Adobe Experience Manager 6.5.4.0 incluem:

* O Adobe Experience Manager Assets agora está configurado com o Brand Portal por meio de [!DNL Adobe I/O] Console do.

* Um novo [Gerar saída para impressão](/help/forms/using/aem-forms-workflow-step-reference.md) A etapa agora está disponível para fluxos de trabalho do Adobe Experience Manager Forms.

![Gerar saída para impressão](/help/release-notes/assets/generate-print-output-step.gif)

* [Suporte a várias colunas](/help/forms/using/resize-using-layout-mode.md) para modo de layout para formulários adaptáveis e Comunicações interativas.
...
* Agora é possível sincronizar subárvores de conteúdo seletivo a *Dynamic Media - Modo Scene7* em vez de todos os disponíveis em `content/dam`.
...
* As estruturas de entrada ou saída SOAP e de dados complexos agora oferecem suporte à Substituição dinâmica de grupos.
...
* Redução do tempo de atualização de uma página após arrastar [!DNL Experience Fragments] para [!DNL Sites] Editor de páginas.

* Reduziu o tempo de carregamento de entradas em um [!DNL Sites] com mais de 200 live copies em **[!UICONTROL Visão geral da Live Copy]**.

* Tratamento de URLs incompletos ou inválidos aprimorado. Esses URLs podem retardar o Editor de modelo.

## Sites {#sites-fixes}

* Quando um URL de uma página do Adobe Experience Manager Sites contém dois pontos (`:`) ou símbolo de porcentagem (`%`), o navegador para de responder e os picos de uso da CPU (NPR-32369, NPR-31918).
...
* Não é possível sincronizar a campanha em Adobe que contém o componente de importador de design OOTB (pronto para uso) (NPR-31728).
...
* A caixa de diálogo de configuração de exibição não funciona corretamente para o recurso de fluxo de trabalho de personalização da Caixa de entrada (NPR-32503 e NPR-32492). Para obter mais informações sobre esse recurso, consulte [Sua Caixa de entrada](/help/sites-authoring/inbox.md).
...
* A geração de representação para ativos com ${extension} falhou (NPR-32294).
...
* A página de detalhes dos ativos do PDF não mostra botões de ação, exceto os botões Para coleta e Adicionar representação no Experience Manager em execução no modo de execução do Dynamic Media Scene7 (CQ-4286705).
...
* O Editor do carrossel não abre no IE 11 no modo híbrido Experience Manager Dynamic Media (CQ-4255590). **Para clientes do Adobe Dynamic Media - Modo híbrido:** O Adobe encerrará o suporte ao Internet Explorer 11 no modo Dynamic Media - Híbrido, após maio de 2022.
...
* Marcação na plataforma: procure tags digitando nos campos de tag para mostrar tags fora dos limites raiz e não respeitar as `rootPath` propriedade de campos de tag (NPR-31895).
...
* A criação de projetos de tradução interrompe a interface do usuário quando a opção Aparar espaços está ativada no `Apache Sling JSP Script Handler` (NPR-32154).
...
* A geração de URL da biblioteca do Launch é baseada somente em `path` e `library_name` valores da API do Launch, e não se baseia em `library_path` (NPR-31550).
...
* Serviços de documento: se o número de transações em um servidor exceder um limite específico, ocorrerá uma falha na conversão de HTML para PDF e as configurações de tipo de arquivo serão removidas de [!DNL Forms] (NPR-32204).
...
* Designer: se houver células mescladas em uma tabela, o teste de acessibilidade falhará para o arquivo de PDF de saída convertido de um formulário XDP usando o serviço de saída (CQ-4285372).
...
O UberJar para o Experience Manager 6.5.4.0 está disponível no [Repositório central Maven](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.4/).
