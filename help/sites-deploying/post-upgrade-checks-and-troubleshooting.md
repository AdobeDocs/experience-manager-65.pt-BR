...
Na sequência da [Atualização no local](/help/sites-deploying/in-place-upgrade.md) as atividades a seguir devem ser executadas para finalizar a atualização. Pressupõe-se que o AEM tenha sido iniciado com o jar 6.5 e que a base de código atualizada tenha sido implantada.
...
Vários recursos no AEM exigem etapas adicionais após a atualização. Uma lista completa desses recursos e etapas para migrá-los no AEM 6.5 pode ser encontrada no site [Atualização de código e personalizações](/help/sites-deploying/upgrading-code-and-customizations.md) página.
...
Executar plano de teste detalhado em relação ao conforme definido [Atualização de código e personalizações](/help/sites-deploying/upgrading-code-and-customizations.md) no **Procedimento de teste** seção.

### Habilitar agentes de replicação {#enable-replication-agents}

Depois que o ambiente de publicação for totalmente atualizado e validado, ative os agentes de replicação no Ambiente de autor. Verifique se os agentes podem se conectar às respectivas instâncias de Publicação. Consulte U [Procedimento de atualização](/help/sites-deploying/upgrade-procedure.md) para obter mais detalhes sobre a ordem dos eventos.
...
A migração de dados do CRX2 para o Oak deve ser viável para qualquer cenário que comece com Instâncias de origem com base no CQ 5.4. Siga exatamente as instruções de atualização neste documento, que incluem a preparação do `repository.xml`, certificando-se de que nenhum autenticador personalizado seja iniciado por meio do JAAS e que a instância tenha sido verificada em busca de inconsistências antes de iniciar a migração.
...
`/apps` e `/libs` são bem tratados pela atualização, mas as alterações em `/etc` pode ser necessário restaurar manualmente a partir de `/var/upgrade/PreUpgradeBackup` após a atualização. Verifique neste local se há conteúdo que precise ser mesclado manualmente.
