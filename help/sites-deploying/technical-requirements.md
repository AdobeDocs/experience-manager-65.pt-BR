...
O Adobe oferece suporte (AEM) ao Adobe Experience Manager nas plataformas, conforme detalhado nas seguintes informações deste documento.

Para qualquer problema relacionado à plataforma, entre em contato com o fornecedor da plataforma.
...
>[!CAUTION]
>
>Rastreie os Boletins de segurança do fornecedor do Java™. Isso garante a segurança dos ambientes de produção. Além disso, sempre instale as atualizações mais recentes do Java™.
...
1. O Oracle foi transferido para um modelo de &quot;Suporte a longo prazo&quot; (LTS) para produtos Oracle Java™ SE. Java™ 9, Java™ 10 e Java™ 12 são versões não LTS do Oracle (consulte [Roteiro de suporte do Oracle Java™ SE](https://www.oracle.com/technetwork/java/eol-135779.html)). Para implantar AEM no ambiente de produção, o Adobe fornece suporte somente para as versões LTS do Java™. O suporte e a distribuição do JDK Java™ SE do Oracle, incluindo todas as atualizações de manutenção de versões LTS além do fim das atualizações públicas, são suportados pelo Adobe diretamente para todos os clientes AEM que usam a tecnologia Oracle Java™ SE. Consulte a [Política de suporte do Java™ para Adobe Experience Manager](assets/Java_Policy_for_Adobe_Experience_Manager.pdf).
**Importante: O Oracle Java™ 11 é compatível, no mínimo, até setembro de 2026. O suporte para Oracle Java™ 17 está em preparação. **

1. O IBM® JRE só é compatível com o WebSphere® Application Server.

1. As versões do Azul Zulu OpenJDK LTS são compatíveis com implantações locais de AEM a partir da versão 6.5 SP9. O suporte e a distribuição das versões Azul Zulu JDK LTS devem ser licenciados diretamente do Azul pelos clientes do Adobe.
...
1. O &#39;Sistema de Arquivos&#39; inclui o armazenamento de blocos compatível com POSIX. Inclui a tecnologia de armazenamento de rede. Considere que o desempenho do sistema de arquivos pode variar e influencia o desempenho geral. Carregar AEM de teste com o sistema de arquivos remoto/de rede.
...
>[!NOTE]
O MongoDB é um software de terceiros e não está incluído no pacote de licenciamento AEM. Para obter mais informações, consulte o [Política de licenciamento do MongoDB](https://www.mongodb.com/community/licensing) página.
...
Caso deseje executar o autor e publicar no MongoDB, duas licenças separadas devem ser compradas.
O Atendimento ao cliente do Adobe auxilia problemas de qualificação relacionados ao uso do MongoDB com AEM.
...
Para executar o AEM 6.5 com um banco de dados relacional suportado, é necessário um contrato de suporte separado com um fornecedor de banco de dados. O Atendimento ao cliente do Adobe auxilia problemas de qualificação relacionados ao uso de bancos de dados relacionais com o AEM 6.5.
...
| IBM® WebSphere® Application Server Continuous Delivery (LibertyProfile) com o Perfil da Web 7.0 e IBM® JRE 1.8 | R: Apoio limitado a novos contratos `[2]` |
...
| JBoss® EAP 7.2.x com servidor de aplicativos JBoss® | Z: Não suportado |
| JBoss® EAP 7.1.4 com servidor de aplicativos JBoss® | R: Apoio limitado a novos contratos `[1]` `[2]` |
| JBoss® EAP 7.0.x com servidor de aplicativos JBoss® | Z: Não suportado |
...
| Microsoft® Windows Server 2019 `[4]` | R: Apoio limitado a novos contratos `[5]` |
...
1. Linux® Kernel 2.6, 3. x, 4. x e 5. x inclui derivados da distribuição Red Hat®, incluindo Red Hat® Enterprise Linux®, CentOS, Oracle Linux® e Amazon Linux®. Os recursos do complemento AEM Forms são suportados apenas no CentOS 7, Red Hat® Enterprise Linux® 7, Red Hat® Enterprise Linux® 8 e Red Hat® Enterprise Linux® 9.
1. O AEM Forms é compatível com o Ubuntu 20.04 LTS.
1. Distribuição Linux® suportada pelos Adobe Managed Services.
1. As implantações de produção do Microsoft® Windows são suportadas para clientes que atualizam para 6.5 e para uso não relacionado à produção. Novas implantações são por solicitação do AEM Sites e do Assets.
...
Em todos os outros casos de implantação de AEM no Azure ou AWS, ou em qualquer outro ambiente de computação em nuvem, o suporte do Adobe é contido no ambiente de computação virtual. Esse ambiente virtual deve ser executado em conformidade com as especificações técnicas listadas nesta página. Qualquer problema relatado relacionado à AEM em execução em qualquer um desses ambientes de nuvem deve ser reproduzível independentemente de qualquer serviço de nuvem específico ao ambiente de computação em nuvem. Ou seja, a menos que o serviço de nuvem seja compatível como parte dos requisitos técnicos listados nesta página, por exemplo, armazenamento do Azure Blob ou AWS S3.

Para recomendações sobre como implantar AEM no Azure ou no AWS, fora do Adobe Managed Services, o Adobe recomenda trabalhar diretamente com o provedor de nuvem. Ou trabalhar com parceiros do Adobe que oferecem suporte à implantação do AEM no ambiente de nuvem de sua escolha. O provedor ou parceiro de nuvem selecionado é responsável pelas especificações de dimensionamento, design e implementação da arquitetura para atender aos requisitos específicos de desempenho, carga, escalabilidade e segurança.
...
1. Os servidores da Web criados com base no código-fonte httpd do Apache têm tanto suporte quanto a versão do httpd na qual se baseia. Em caso de dúvida, solicite ao Adobe a confirmação do nível de suporte relacionado ao respectivo produto de servidor. Os seguintes casos:

   1. O servidor HTTP foi criado usando apenas distribuições oficiais de origem do Apache ou
   1. O servidor HTTP foi entregue como parte do sistema operacional no qual está sendo executado. Exemplos: Servidor HTTP IBM®, Servidor HTTP Oracle
...
1. Versão de suporte estendida do Firefox [Saiba mais em mozilla.org](https://www.mozilla.org/en-US/firefox/enterprise/)
...
Ao se conectar com o Microsoft® Windows 7+ a uma instância de AEM que não esteja protegida com SSL, a autenticação básica por rede não segura deve ser ativada no Windows. Requer uma alteração no Registro do Windows do WebClient:
...
A operação é contínua, pois nenhuma configuração especial é necessária. Especifique um endereço IP usando o formato adequado ao seu tipo de rede, se necessário.

Quando um endereço IP deve ser especificado, você pode selecionar (conforme necessário) dentre as seguintes opções:
...
Para clientes do Dynamic Media que executam o Dynamic Media - modo Scene7 (ou seja, **dynamicmedia_scene7** modo de funcionamento), não existem requisitos adicionais de sistema; apenas os mesmos requisitos de sistema AEM. Dynamic Media - A arquitetura de modo Scene7 usa o serviço de imagem baseado em nuvem e não o serviço incorporado no AEM.
...
>[!NOTE]
**Arquitetura NUMA:** Os sistemas com processadores AMD64 e Intel® EM64T são configurados normalmente como plataformas não uniformes de arquitetura de memória (NUMA). Ou seja, o kernel constrói vários nós de memória no momento da inicialização em vez de construir um único nó de memória.
...
Para usar o Dynamic Media no Windows, instale o Microsoft® Visual Studio 2010, 2013 e 2015 redistribuíveis para x64 e x86.
...
<table>
 <tbody>
  ...
  <tr>
   <td><a href="https://helpx.adobe.com/acrobat/release-note/release-notes-acrobat-reader.html">Acrobat 2020 classic track</a> versão mais recente</td>
   <td>XPS, formatos de imagem (BMP, GIF, JPEG, JPG, TIF, TIFF, PNG, JPX, JPX, JP2, J2K, J2C, JPC), HTML, HTM, DWG, DXF e DWF</td>
  </tr>
  <tr>
   <td><a href="https://helpx.adobe.com/acrobat/release-note/release-notes-acrobat-reader.html">Acrobat 2017 classic track</a> versão mais recente (obsoleta)</td>
   <td>XPS, formatos de imagem (BMP, GIF, JPEG, JPG, TIF, TIFF, PNG, JPX, JPX, JP2, J2K, J2C, JPC), HTML, HTM, DWG, DXF e DWF</td>
  </tr>
  <tr>
  ...
  <tr>
   <td>Editor Microsoft® 2019<br /> </td>
   <td>PUB</td>
  </tr>
  <tr>
...
* O Gerador de PDF suporta apenas a versão de varejo de 32 bits do Microsoft® Office Professional Plus e outros softwares necessários para a conversão no sistema operacional Windows.
* PDF Generator suporta as versões de 32 e 64 bits do OpenOffice no sistema operacional Linux®.
...
* Uma versão do Acrobat é fornecida com o AEM Forms para habilitar a funcionalidade PDF Generator. Acesse programaticamente a versão fornecida somente com o AEM Forms, durante o período da licença do AEM Forms, para uso com o PDF Generator da AEM Forms. Para obter mais informações, consulte a descrição do produto AEM Forms de acordo com sua implantação ([No local](https://helpx.adobe.com/legal/product-descriptions/adobe-experience-manager-on-premise.html) ou [Managed Services](https://helpx.adobe.com/legal/product-descriptions/adobe-experience-manager-managed-services.html))
* O serviço PDF Generator não suporta Microsoft® Windows 10.
* PDF Generator falha ao converter arquivos usando o Microsoft® Visio 2019. Você pode continuar usando o Microsoft® Visio 2016 para converter `.VSD` e `.VSDX` arquivos.
* PDF Generator falha ao converter arquivos usando o Microsoft® Project 2019. Você pode continuar usando o Microsoft® Project 2016 para converter `.VSD` e `.VSDX` arquivos.
>
...
* **Sistemas operacionais:**
   ...
   * Linux® (suporte a aplicativos de 32 e 32 bits em sistemas de 64 bits). Para obter etapas para instalar bibliotecas de clientes de 32 bits, consulte [Como habilitar XMP extração e write-back no Red Hat® Linux® de 64 bits](https://helpx.adobe.com/experience-manager/kb/enable-xmp-write-back-64-bit-redhat.html).
