...
No AEM Communities, a partir do ambiente de publicação (dependendo das permissões configuradas), os visitantes *do* site podem se tornar *membros*, criar grupos *de* usuários e editar seu perfil *de* membro.
...
* **pacote** vlt: é um arquivo zip de todas as alterações feitas em um editor, que precisam ser distribuídas entre os editores. As alterações em um editor geram eventos que são selecionados pelo ouvinte de eventos de alteração. Isso cria um pacote vlt que contém todas as alterações.
...
Quando a topologia é um farm [de](/help/communities/topologies.md) publicação e a sincronização do usuário foi configurada corretamente, o perfil *de* usuário *e* usuário é sincronizado no farm de publicação usando a distribuição Sling.
...
1. Localize **Apache Sling Distribution Agent - Fábrica de agentes de sincronização.**
...
1. Localize o Agente de distribuição **Apache Sling - Fábrica de agentes da fila.**
1. Selecione a configuração existente para abrir para edição (ícone de lápis).
...
Para problemas na distribuição Sling em que há uma discrepância nas assinaturas e seguintes, verifique se as seguintes propriedades nas configurações do **AEM Communities User Sync Listener** estão definidas:
...
Em **/home/users**, um nó ***/rep:cache **é criado. É um cache que é usado para descobrir que, se consultarmos o nome principal de um nó, esse cache poderá ser usado diretamente.
...
1. Adicionar dois filtros de nó do pacote:
   * /home/users|-.*/.tokens
...
1. **Verifique as configurações**. Certifique-se de que todas as [](/help/communities/sync.md#bestpractices)configurações estejam definidas adequadamente na instância do autor de AEM, conforme mencionado nas [Práticas recomendadas](/help/communities/sync.md#main-pars-header-863110628).
