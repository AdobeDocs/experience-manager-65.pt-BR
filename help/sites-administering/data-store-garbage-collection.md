...
seo-description: Saiba como configurar a Coleta de lixo do armazenamento de dados para liberar espaço em disco.
...
## Executando a Coleta de Lixo do Data Store {#running-data-store-garbage-collection}
...
1. Por meio de [Revisão de limpeza](/help/sites-deploying/revision-cleanup.md) - um mecanismo de coleta de lixo normalmente usado para limpeza de armazenamento de nó.

1. Por meio de [Data Store Garbage Collection](/help/sites-administering/data-store-garbage-collection.md#running-data-store-garbage-collection-via-the-operations-dashboard) - um mecanismo de coleta de lixo específico para armazenamentos de dados externos, disponível no Painel de Operações.
...
A Janela de manutenção semanal integrada, disponível por meio do [Operations Dashboard](/help/sites-administering/operations-dashboard.md), contém uma tarefa incorporada para acionar a coleta de lixo do armazenamento de dados às 1:00 dos domingos.
...
1. Selecione a tarefa **Data Store Garbage Collection** e clique ou toque no ícone **Run**.
...
>[!NOTE]
>
>A tarefa Coleta de lixo do armazenamento de dados só estará visível se você tiver configurado um armazenamento de dados de arquivo externo. Consulte [Configuração de armazenamentos de nó e armazenamentos de dados em AEM 6](/help/sites-deploying/data-store-config.md#file-data-store) para obter informações sobre como configurar um armazenamento de dados de arquivo.
...
Esta seção trata da execução manual da coleta de lixo do armazenamento de dados por meio do Console JMX. Se a instalação estiver configurada sem um armazenamento de dados externo, isso não se aplica à instalação. Em vez disso, consulte as instruções sobre como executar a limpeza de revisão em [Manutenção do Repositório](/help/sites-deploying/storage-elements-in-aem-6.md#maintaining-the-repository).
...
1. No Console de Gerenciamento OSGi do Apache Felix, realce a guia **Main** e selecione **JMX** no menu a seguir.
...
1. Clique em **Invoke**. O CRX executa a coleta de lixo e indica quando ela foi concluída.
...
>[!CAUTION]
>
>No exemplo a seguir, comandos `curl` podem ser necessários vários parâmetros para a sua instância; por exemplo, o nome do host ( `localhost`), a porta ( `4502`), a senha do administrador ( `xyz`) e vários parâmetros para a coleta de lixo do armazenamento de dados real.
...
Após a conclusão da verificação de consistência, uma mensagem mostrará o número de binários relatados como ausentes. Se o número for maior que 0, verifique `error.log` para obter mais detalhes sobre os binários ausentes.
