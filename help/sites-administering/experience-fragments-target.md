...
>* O AEM deve ser [integrado ao Adobe Target usando a E/S](/help/sites-administering/integration-ims-adobe-io.md)da Adobe.
>
>
AEM 6.5.0.0 e 6.5.1.0:
>
>* Os Fragmentos de experiência do AEM são exportados para a área de trabalho padrão do Adobe Target.
>* O AEM deve ser integrado ao Adobe Target de acordo com as instruções em [Integração com o Adobe Target](/help/sites-administering/target.md).


Você pode exportar Fragmentos [de](/help/sites-authoring/experience-fragments.md)experiência criados no Adobe Experience Manager (AEM) para Adobe Target (Público alvo). Eles podem ser usados como ofertas em atividades Públicos alvos, para testar e personalizar experiências em escala.

Há três opções de formato disponíveis para exportar um Fragmento de experiência para Adobe Target:

* HTML (padrão): Suporte para delivery de conteúdo da Web e híbrido
* JSON: Suporte para delivery de conteúdo sem cabeçalho
* HTML e JSON

Os Fragmentos de experiência do AEM podem ser exportados para o espaço de trabalho padrão no Adobe Target ou para espaços de trabalho definidos pelo usuário para o Adobe Target. Isso é feito por meio da E/S da Adobe, para a qual o AEM deve ser [integrado ao Adobe Target usando a E/S](/help/sites-administering/integration-ims-adobe-io.md)da Adobe.

>[!NOTE]
>
>Os espaços de trabalho Adobe Target não existem no Adobe Target. Eles são definidos e gerenciados no Adobe IMS (Identity Management System) e, em seguida, selecionados para uso em soluções que usam integrações de E/S da Adobe.

>[!NOTE]
>
>Os espaços de trabalho de Adobe Target podem ser usados para permitir que membros de uma organização (grupo) criem e gerenciem ofertas e atividades apenas para essa organização; sem dar acesso a outros usuários. Por exemplo, organizações específicas a cada país dentro de uma preocupação global.

>[!NOTE]
>
>Para obter mais informações, consulte também:
>
>* [Adobe Target](https://www.adobe.io/apis/experiencecloud/target.html)
...
1. É necessário [integrar o AEM ao Adobe Target usando a E/S](/help/sites-administering/integration-ims-adobe-io.md)da Adobe.
2. Os Fragmentos de experiência são exportados da instância do autor de AEM, portanto, é necessário [Configurar o Externalizador](/help/sites-administering/target-requirements.md#configuring-the-aem-link-externalizer) de link de AEM na instância do autor para garantir que qualquer referência no Fragmento de experiência seja externalizada para o delivery da Web.
...
1. 
   ...
   * se necessário - o domínio do externalizador
   >[!CAUTION]
   >
   >O domínio do externalizador é opcional. Um externalizador do AEM é configurado quando você deseja que o conteúdo exportado aponte para um domínio de *publicação* específico. Para obter mais detalhes, consulte [Configuração do Externalizador](/help/sites-administering/target-requirements.md#configuring-the-aem-link-externalizer)de links AEM.

   Por exemplo, para uma pasta:

   ![Pasta - Cloud](assets/xf-target-integration-01.png "ServicesFolder - Cloud Service")
...
>[!CAUTION]
>
>Para ativos de mídia, como imagens, apenas uma referência é exportada para o Público alvo. O ativo em si permanece armazenado em AEM Assets e é entregue da instância de publicação do AEM.
>
...
Para exportar um fragmento de experiência do AEM para o Público alvo (depois de especificar a Configuração da nuvem):
...
1. Toque/clique em **OK** na caixa de diálogo de confirmação.
   ...
   >[!NOTE]
   >
   >Ao exibir um Fragmento de experiência no Adobe Target, a *última data modificada* que é vista é a data em que o fragmento foi modificado pela última vez no AEM, não a data em que o fragmento foi exportado pela última vez para Adobe Target.
...
Depois de executar as tarefas anteriores, o fragmento de experiência é exibido na página Ofertas no Target. Consulte a documentação [](https://experiencecloud.adobe.com/resources/help/en_US/target/target/aem-experience-fragments.html) específica do Público alvo para saber mais sobre o que você pode alcançar.
...
A exclusão de um fragmento de experiência que já foi exportado para o Público alvo pode causar problemas se o fragmento já estiver sendo usado em uma oferta no Público alvo. A exclusão do fragmento tornaria a oferta inutilizável, pois o conteúdo do fragmento estava sendo entregue pelo AEM.
...
* Se o Fragmento de experiência estiver sendo usado por uma atividade no Público alvo, uma mensagem de erro avisará o usuário do AEM sobre possíveis consequências que a exclusão do fragmento terá na atividade.
   ...
   * A oferta do Público alvo com o Fragmento de experiência do AEM pode mostrar um comportamento indesejado
   ...
   * É claro que quaisquer modificações adicionais no Fragmento de experiência são impossíveis, pois o Fragmento de experiência não existe mais no AEM.