...
Verifique se as chamadas HTTP são enviadas e recebidas por ambas as soluções (AEM > Adobe Campaign Classic, Adobe Campaign Classic > AEM). Isso evita problemas de firewall/SSL.

* Para a funcionalidade AEM, você verá que as chamadas JSON são solicitadas da interface do autor do AEM
   * Eles não devem resultar em um erro HTTP-500.
   * Se você vir erros HTTP-500, verifique a `error.log` para obter mais informações sobre isso.
...
Verifique a fonte do HTML e confirme se você pode abrir o URL da máquina cliente. Se o URL tiver `localhost:4503` Nele, altere a configuração do Day CQ Link Externalizer na instância do autor do AEM para apontar para uma instância de publicação que pode ser acessada no computador do console do Adobe Campaign Classic.
...
* Se ele não estiver instalado, você poderá corrigir o `/usr/local/neolane/nl6/env.sh` script e altere o local para um local instalado.
...
Isso acontece se houver informações desatualizadas do Adobe Campaign Classic na instância AEM. Você pode resolver esse problema ao:

1. Remoção de todas as configurações de integração do Adobe Campaign Classic que estão no AEM.
1. Reconstruindo a integração.
...
Crie um tíquete com a equipe de suporte da Adobe Campaign se você vir o seguinte na `error.log` do AEM.
...
Ao tentar sincronizar o conteúdo no delivery do Adobe Campaign Classic, o AEM retorna uma lista de boletins informativos. No entanto, os URLs para os boletins informativos na lista podem ser endereços HTTP em vez de HTTPS. Ocorre um erro ao selecionar um dos itens na lista. Isso pode ocorrer com a seguinte configuração.
...
* O AEM dispatcher ou proxy reverso precisa ser configurado para transmitir o protocolo original como um cabeçalho.
* A variável **Filtro SSL do serviço Apache Felix Http** na configuração do OSGi do AEM precisa ser definido com as configurações do cabeçalho necessárias.
   * `https://<host>:<port>/system/console/configMgr`
   * Consulte [https://felix.apache.org/documentation/subprojects/apache-felix-http-service.html#using-the-ssl-filter](https://felix.apache.org/documentation/subprojects/apache-felix-http-service.html#using-the-ssl-filter)
...
Ao criar um template de email no AEM para Adobe Campaign Classic, você deve incluir a propriedade `acMapping` com o valor `mapRecipient` no `jcr:content` do modelo, ou você não poderá selecionar o modelo Adobe Campaign Classic em **Propriedades da página** do AEM. O campo aparecerá desativado.
...
Esse é um problema em que, se a variável `acMapping` estiver definida com um valor diferente de `recipient.firstName`, um valor em branco é criado no Adobe Campaign Manager.

Se isso ocorrer, instale o pacote de recursos 6576 para AEM da [Compartilhamento de pacotes](/help/sites-administering/package-manager.md#package-share).
