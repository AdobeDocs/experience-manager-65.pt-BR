...
Para usar o Dynamic Media Classic, é necessário configurar a configuração de nuvem para que o Dynamic Media Classic e os ativos AEM possam interagir uns com os outros. Este documento descreve como configurar o AEM e o Dynamic Media Classic.
...
* Se você for um cliente **existente** do Dynamic Media Classic cujos ativos de mídia avançada residem no Dynamic Media Classic para publicação e delivery, mas quiser integrar esses ativos à criação do Sites (WCM) e/ou aos ativos AEM para gerenciamento, use a integração [ponto a ponto do](#aem-scene-point-to-point-integration) AEM/Dynamic Media Classic descrita neste documento.
...
1. Siga as instruções específicas do caso de uso:
   ...
   <table>
    ...
    <tr>
    <td>No varejo e são novidade no Sites e no Dynamic Media</td>
    ...
    <td>Faça upload de ativos para os ativos AEM e use o URL/código incorporado publicado do Dynamic Media</td>
    <td><p>Ligado</p> <p>(Consulte a etapa 3)</p> </td>
    <td>Desativado</td>
    <td>Desativado</td>
    <td>Desativado</td>
...
A partir do AEM 6.3.2, os ativos AEM agora são configurados para você para que todos os ativos digitais carregados no gerenciador de ativos digitais sejam automaticamente atualizados para o Dynamic Media Classic se os ativos estiverem em uma pasta de público alvo CQ.
...
1. No sidekick, navegue até os componentes do **[!UICONTROL Fluxo de trabalho]** e selecione **[!UICONTROL Dynamic Media Classic]**. Arraste o **[!UICONTROL Dynamic Media Classic]** para o fluxo de trabalho e toque em **[!UICONTROL Salvar]**. Os ativos adicionados aos ativos AEM na pasta do público alvo serão carregados automaticamente para o Dynamic Media Classic.
...
Os ativos do Dynamic Media Classic permanecem disponíveis por pré-visualização segura. Somente quando os ativos são publicados no AEM, os ativos do Dynamic Media Classic também entram em produção.
...
* Verifique se os ativos estão na pasta do público alvo CQ. Somente os ativos que estão na pasta do público alvo CQ são atualizados automaticamente (desde que você tenha configurado os ativos AEM para fazer upload automático dos ativos).
