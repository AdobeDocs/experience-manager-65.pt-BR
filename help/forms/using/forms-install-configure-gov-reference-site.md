...
* **we-gov-forms.pkg.all-&lt;version>.zip**  - Pacote de demonstração  *completo*
   ...
   * **we-gov-forms.ui.apps-&lt;version>.zip** *- Contém todos os componentes, bibliotecas de clientes, usuários de exemplo, modelos de fluxo de trabalho etc.*
      ...
      * **we-gov-forms.core-&lt;version>.jar**  -  *Contém todos os serviços OSGI, implementação de etapa do fluxo de trabalho personalizado etc.*
...
1. Toque no nome do pacote **we-gov-forms.pkg.all-&lt;version>.zip**, selecione **[!UICONTROL Aceitar Termos EULA]** e toque em **[!UICONTROL Download]**.
...
1. Navegue até *https://&lt;aemserver>:&lt;port>/crx/packmgr/index.jsp*
1. Localize o arquivo mais antigo **we-gov-forms.pkg.all-&lt;version>.zip**.
...
1. Selecione na lista suspensa &quot;**We.Gov Forms Users**&quot;.
...
1. Repita as etapas de 2 a 7 pesquisando por &quot;**analytics**&quot;, selecionando o grupo &quot;**Analytics Administrators**&quot; e adicionando o grupo &quot;**We.Gov Forms Users**&quot; como membro.
1. Repita as etapas de 2 a 7 pesquisando por &quot;**forms users**&quot;, selecionando o grupo &quot;**forms-power-users**&quot; e adicionando o grupo &quot;**We.Gov Forms Users**&quot; como membro.
1. Repita as etapas de 2 a 7 procurando por &quot;**forms-users**&quot;, selecionando o grupo &quot;**forms-users**&quot; e, desta vez, adicionando o grupo &quot;**We.Gov Users**&quot; como membro.

### Configuração do servidor de email {#email-server-configuration}

1. Revise a documentação de configuração [Configuração de Notificação por Email](/help/sites-administering/notification.md)
...
1. O pacote `we-gov-forms.pkg.all-[version].zip` inclui uma chave SSL de amostra e um certificado que podem ser acessados ao extrair a pasta `we-gov-forms.pkg.all-[version].zip/ssl` que faz parte do pacote.
...
1. Vá até:
   ...
   *https://&lt;aemserver>: &lt;port>/libs/adobesign/cloudservices/adobesign.html/conf/we-gov*
   ...
   >[!NOTE]
   >
   >O URL usado para acessar o servidor de AEM deve corresponder ao URL configurado no URI de redirecionamento do Adobe Sign OAuth para evitar problemas de configuração (por exemplo, *https://&lt;aemserver>:&lt;port>/mnt/overlay/adobesign/cloudservices/adobesign/properties.html*)
...
1. Selecione a configuração &quot;Cloud Service do Microsoft Dynamics OData&quot;.
1. Clique em &quot;Propriedades&quot;.
...
1. Insira os seguintes detalhes:
   ...
   1. **Configurações de autenticação**  (consulte as configurações de configuração de nuvem do  [MS Dynamics ](../../forms/using/forms-install-configure-gov-reference-site.md#dynamicsconfig) para coletar essas informações):
...
1. Copie o **Application ID** a ser usado como o OAuth **Client Id** na configuração da nuvem de AEM
...
1. Teste os serviços para garantir que eles se conectem com êxito à Fonte de Dados configurada.
   ...
   >[!NOTE]
   Após testar os serviços, clique em **Cancel** para garantir que as alterações involuntárias não sejam propagadas para o Modelo de dados de formulário.
...
1. Selecione **FDM de Inscrição We.gov** e selecione **Propriedades**.

   ![Propriedades do Dynamics CRM FDM](assets/aftia-enrollment-fdm.jpg)

1. Navegue até a guia **Atualizar Origem**.

1. Certifique-se de que a **Configuração sensível ao contexto** esteja definida como `/conf/we-gov` e que a fonte de dados configurada seja **We.Gov Derby DS**.

   ![Propriedades do Dynamics CRM FDM](assets/aftia-update-data-source.jpg)

1. Clique em **Salvar e fechar**.

1. [Testar os ](work-with-form-data-model.md#test-data-model-objects-and-services) serviços para garantir que eles se conectem com êxito à Fonte de Dados configurada

   * Para testar a conexão, selecione o **HOMEMORTGAGEACCOUNT** e forneça um serviço de obtenção. Teste o serviço e os administradores do sistema podem ver os dados sendo recuperados.
...
>[!NOTE]
Os dados do AEM Forms Analytics estão disponíveis offline ou sem uma configuração de nuvem do Adobe Analytics se o pacote `we-gov-forms.ui.analytics-<version>.zip` estiver instalado, mas os dados do AEM Sites exigirem uma configuração de nuvem ativa.
...
No canto superior esquerdo > Ferramentas > Segurança > Configuração do Adobe IMS.
...
1. Não continue com o restante da configuração - reveja a seção [Criando Integração no Adobe I/O](#create-integration-adobeio)
...
1. Navegue até [Console Adobe I/O](https://console.adobe.io/).
...
1. Abra seu navegador e navegue até a URL do sistema https://&lt;nome_do_domínio>:&lt;porta_do_sistema>
...
1. Para essa configuração, deixamos os dois valores de caixa de seleção vazios.
   ...
   * Para entender mais sobre essas opções, consulte [Configurar o serviço de nuvem](https://docs.adobe.com/content/help/en/aem-forms-automated-conversion-service/using/configure-service.html#configure-the-cloud-service).
...
1. Navegue até o sistema de AEM https://&lt;nome_do_domínio>:&lt;porta_do_sistema>
...
Você também pode consultar [Templates customization style system](../../forms/using/forms-install-configure-gov-reference-site.md#customizetemplates) para obter a documentação sobre estilos suportados.
...
O Formulário adaptável de inscrição é enviado para um fluxo de trabalho OSGI para processamento. Este workflow pode ser encontrado em *https://&lt;aemserver>:&lt;port>/conf/we-gov/settings/models/we-gov-process.html*.
