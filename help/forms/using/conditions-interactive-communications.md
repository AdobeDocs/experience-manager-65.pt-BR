...
1. Toque nos ativos apropriados para selecioná-los para incluírem na condição e toque em **[!UICONTROL Concluído]**.
   ...
   **[] Alteração do AReject.** Toque nesse ícone para rejeitar as alterações que você pode ter feito no ativo e na regra na condição.
   **[] Alteração de BAccept.** Toque nesse ícone para aceitar as alterações feitas no ativo e na regra na condição.
   **[] Ativo duplicado.** Toque neste ícone para criar uma cópia do ativo junto com a regra aplicada, se houver, na condição. Em seguida, você pode continuar a editar a regra e o ativo para o ativo duplicado. A duplicação de um ativo é útil para criar regras semelhantes para exibir ativos alternativos com base em um contexto específico.
   **[] Exibir visualização DS.** Toque neste ícone para exibir uma pré-visualização do ativo na página Criar\Editar condição.
