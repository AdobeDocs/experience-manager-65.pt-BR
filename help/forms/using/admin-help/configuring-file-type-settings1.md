...
**Idioma OCRL Primário:** O idioma a ser usado pelo mecanismo de OCR para identificar os caracteres. O padrão é inglês (EUA).
...
[Configurações de projeto do Microsoft (somente Windows)](/help/forms/using/admin-help/configuring-file-type-settings1.md#microsoft-project-settings-windows-only)
...
A configuração Tipo de arquivo de exportação na seção de configurações de exportação do Adobe PDF é usada para converter um arquivo PDF em outro formato. O padrão é HTML 4.01 com folhas de estilos em cascata (CSS) 1.0(&amp;ast;.htm, &amp;ast;.html).
...
1. Para incorporar apenas os subconjuntos em uso das fontes inseridas, selecione **Subdefinir todas as fontes inseridas**.
   ...
   >[!NOTE]
   >
   >*Se você estiver usando esta opção em combinação com **Desincorporar algumas fontes**, as fontes da lista **A**adicionar fontes para desincorporar ainda serão completamente desincorporadas.*
...
Essas opções determinam como os arquivos do AutoCAD são convertidos. Para obter instruções sobre como acessar essas opções, consulte [Criar ou editar configurações de tipo de arquivo](/help/forms/using/admin-help/configuring-file-type-settings1.md#create-or-edit-file-type-settings).
