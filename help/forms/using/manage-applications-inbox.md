...
Uma das muitas maneiras de iniciar ou acionar um fluxo de trabalho centrado no Forms é por meio de aplicativos em AEM Caixa de entrada. Para disponibilizar um fluxo de trabalho do Forms como aplicativo na Caixa de entrada, crie um aplicativo de fluxo de trabalho. Para obter mais informações sobre o aplicativo de fluxo de trabalho e outras maneiras de iniciar fluxos de trabalho do Forms, consulte [Iniciar um fluxo de trabalho centrado na Forms no OSGi](../../forms/using/aem-forms-workflow.md#launch).
...
1. Toque **Abrir** para exibir detalhes da tarefa. A exibição detalhada da tarefa é aberta. Nesta visualização, você pode visualizar os detalhes da tarefa e trabalhar na tarefa.
...
À medida que você trabalha na tarefa, ela prossegue no fluxo de trabalho.

### Exibir tarefas concluídas {#view-completed-tasks}

AEM Caixa de entrada exibe somente as tarefas ativas. Tarefas concluídas não aparecem na lista. No entanto, você pode usar filtros da Caixa de entrada para filtrar tarefas com base em vários parâmetros, como tipo de tarefa, status e datas de início e término. Para exibir tarefas concluídas:
...
A tarefa é aberta para exibir o documento ou o formulário adaptável associado à tarefa. Para um formulário adaptável, a tarefa exibe o formulário adaptável somente leitura ou seu documento PDF de registro como configurado na guia Formulário/Documento do [Etapa Atribuir fluxo de trabalho da tarefa](/help/sites-developing/workflows-step-ref.md).
...
1. Use um dos seguintes métodos para adicionar índices:
   ...
   * Implante os índices por meio de um pacote de AEM. Você pode usar um [Arquétipo de AEM](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/using.html?lang=en) projeto para criar um pacote AEM implantável. Use o código de amostra a seguir para adicionar índices a um projeto AEM Archetype:
...
1. [Criar um Índice de propriedades e defini-lo como verdadeiro](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/deploying/queries-and-indexing.html#the-property-index).
