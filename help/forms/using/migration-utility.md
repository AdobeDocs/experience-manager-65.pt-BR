...
Devido a [relacionado à compatibilidade com versões anteriores](/help/sites-deploying/backward-compatibility.md) for alterada, os locais de algumas pastas no repositório crx serão alterados. Exporte e importe manualmente as dependências (bibliotecas e ativos personalizados) da configuração anterior para um novo ambiente.
...
1. Faça o seguinte para executar a migração:
   ...
   * Quando os ativos são atualizados: Os ativos são atualizados com sucesso.
