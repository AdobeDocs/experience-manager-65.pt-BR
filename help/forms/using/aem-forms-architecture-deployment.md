...
* **Processamento de dados de formulário brutos provenientes de uma instância do Publish:** Isso é feito principalmente em uma instância de Processamento através de fluxos de trabalho AEM que são acionados quando os dados chegam. Os workflows podem usar a etapa Modelo de dados de formulário fornecida pronta para uso para arquivar os dados ou documentos em um armazenamento de dados adequado.
* **Armazenamento seguro de dados de formulário**: o processamento fornece um repositório atrás do firewall para dados brutos de formulário que são isolados dos usuários. Nem os designers de formulário na instância do Autor nem os usuários finais na instância do Publish podem acessar esse repositório.
...
* **Armazenamento e pós-processamento de dados de correspondência provenientes de uma instância do Publish:** os fluxos de trabalho AEM executam o pós-processamento opcional das definições de correspondência correspondentes. Esses workflows podem salvar os dados processados finais em um armazenamento de dados externo adequado.
...
A topologia é recomendada para os clientes que planejam usar o AEM Forms no servidor JEE para recursos de gerenciamento de processo (HTML Workspace) sem usar nenhum recurso de pós-processamento, formulários adaptáveis, formulários HTML5 e comunicação interativa.
