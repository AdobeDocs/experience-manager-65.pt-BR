...
* Uma instância AEM está ativa e em execução. Para usuários do Windows, instale a instância AEM no modo elevado. Na terminologia AEM, uma &quot;instância&quot; é uma cópia da AEM em execução em um servidor no modo de autor ou publicação. Você precisa de pelo menos duas [AEM instâncias (um Autor e uma Publicação)](/help/sites-deploying/deploy.md) para executar os recursos de captura de dados da AEM Forms:
...
### (Somente para Windows) Instalação automática do Visual Studio redistributables {#automatic-installation-visual-studio-redistributables}
...
Para avaliar se os redistribuíveis do Visual Studio são instalados automaticamente, abra o arquivo `error.log` disponível no diretório `/crx-repository/logs/`. Os registros incluem a seguinte mensagem:
...
1. Abra o arquivo `[AEM installation directory]\crx-quickstart\conf\sling.properties` para edição.
   ...
   Se você usou `[AEM installation directory]\crx-quickstart\bin\start.bat` para start AEM, edite sling.properties localizado em `[AEM_root]\crx-quickstart\`.
