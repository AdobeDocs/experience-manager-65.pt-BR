...
Instale os pacotes de sites de referência We.Gov e We.Finance usando a Distribuição [de](https://docs.adobe.com/content/help/en/experience-cloud/software-distribution/home.html)software. Para obter mais informações, consulte [Implantar pacotes](#refsite)de sites de referência.
...
* **AEM essenciais** AEM QuickStart, pacote de complementos AEM Forms e pacotes de sites de referência. Consulte as versões [da](https://helpx.adobe.com/br/aem-forms/kb/aem-forms-releases.html) AEM Forms para obter detalhes sobre pacotes de complementos e sites de referência.

* **Um serviço** SMTP Você pode usar qualquer serviço SMTP.

* **Conta de desenvolvedor Adobe Sign e aplicativo** Adobe Sign API Para usar os recursos de assinatura digital, é necessária uma conta de desenvolvedor Adobe Sign. Consulte [Adobe Sign](https://acrobat.adobe.com/us/en/why-adobe/developer-form.html).
...
* Uma instância em execução do AEM com o pacote complementar Forms. Para obter mais informações, consulte [Instalação e configuração do AEM Forms](../../forms/using/installing-configuring-aem-forms-osgi.md).
...
AEM configurações do serviço DS são necessárias na instância Publicar para comunicações por email nos casos de uso do site de referência. Para obter etapas detalhadas para configurar AEM configuração do Serviço DS na instância Publicar, consulte [Configurar AEM configurações](../../forms/using/configuring-the-processing-server-url-.md)do DS.
...
1. AEM instância do autor, vá para **Ferramentas** > **Cloud Services** > Fontes **** de dados > **global**. Toque no ícone **Refsite Dynamics Integration** e toque em Propriedades.
...
1. Forneça suas credenciais de logon. Depois de conectado, você é redirecionado para a página de configuração do serviço de nuvem da AEM Forms. Clique em **Salvar e fechar**. A configuração do serviço de nuvem é salva.
1. Vá para **Forms** > Integrações **** de dados > **We.Finance**. Selecione Seguro automático (Dinâmico) e clique em Editar. As entidades do Microsoft Dynamics estão listadas na guia Fontes de Dados. Aguarde até que todas as entidades sejam buscadas no Microsoft Dynamics e listadas na guia fontes de dados.
...
1. Vá para **Ferramentas** > **Cloud Services** > **Adobe Sign** > **global**. Selecione **AEM Forms Reference Site Sign** e toque em Propriedades.

   >[!CAUTION]
   >
   >Certifique-se de que o `https://[host]:[ssl_port]/mnt/overlay/adobesign/cloudservices/adobesign/properties.html` URL seja adicionado à lista de URL de redirecionamento da configuração OAuth do aplicativo da API do Adobe Sign.
...
1. Start o autor para iniciar rapidamente com o argumento jvm `-Dabtesting.enabled=true` para ativar o teste A/B no servidor.
   ...
   >[!NOTE]
   >
   >Se a instância AEM estiver em execução no JBoss, que é iniciado como um serviço da instalação do Turnkey, adicione o `-Dabtesting.enabled=true` parâmetro na seguinte entrada no `jboss\bin\standalone.conf.bat` arquivo:
