---
title: Invocar AEM Forms usando Remota
seo-title: Invocar AEM Forms usando Remota
description: 'null'
...
# Invocar AEM Forms usando Remota {#invoking-aem-forms-using-remoting}

Os processos criados no Workbench podem ser chamados usando o Remoting. Ou seja, você pode invocar um processo AEM Forms de um aplicativo cliente criado com o Flex. Esse recurso tem por base os Serviços de dados.

>[!NOTE]
>
>Ao usar o Remoting, recomenda-se que você chame processos criados no Workbench em vez dos serviços do AEM Forms. No entanto, é possível invocar diretamente os serviços AEM Forms. (Consulte Criptografar documentos PDF usando a Remoção localizada no Centro de desenvolvedores do AEM Forms.)
...
O seguinte processo de vida curta AEM Forms, nomeado `MyApplication/EncryptDocument`, pode ser chamado usando o Remoting. (Para obter informações sobre esse processo, como seus valores de entrada e saída, consulte o exemplo [de processo de duração](/help/forms/developing/aem-forms-processes.md)curta.)

![iu_iu_encryptdocumentprocess2](assets/iu_iu_encryptdocumentprocess2.png)

>[!NOTE]
>
>Para chamar um processo de AEM Forms usando um aplicativo Flex, verifique se um terminal remoto está habilitado. Por padrão, um terminal remoto é ativado ao implantar um processo.
...
>[!NOTE]
>
>O `MyApplication/EncryptDocument` processo não se baseia em um processo de AEM Forms existente. Para seguir junto com os exemplos de código, crie um processo chamado `MyApplication/EncryptDocument` usando o Workbench.
...
[Tratamento de documentos com (obsoletos para formulários AEM) AEM Forms Remota](invoking-aem-forms-using-remoting.md#handling-documents-with-remoting)

[Invocar um processo de duração curta transmitindo um documento não seguro usando (obsoleto para formulários AEM) AEM Forms Remota](invoking-aem-forms-using-remoting.md#invoking-a-short-lived-process-by-passing-an-unsecure-document-using-remoting)
...
Para obter informações sobre como exibir dados de processo em um controle de gráfico Flex, consulte [Exibição de dados de processo de AEM Forms em gráficos](https://www.adobe.com/devnet/livecycle/articles/populating_flexcontrols.html)Flex.

>[!NOTE]
>
>*Certifique-se de colocar o arquivo crossdomain.xml no local apropriado. Por exemplo, supondo que você implantou AEM Forms em JBoss, coloque esse arquivo no seguinte local: &lt;install_diretory>\Adobe_Experience_Manager_forms\jboss\server\lc_turnkey\deploy\jboss-web.deployer\ROOT.war.*
...
[Invocar AEM Forms usando (obsoleto para formulários AEM) AEM Forms Remota](invoking-aem-forms-using-remoting.md#invoking-aem-forms-using-remoting)
...
Um dos mais importantes tipos Java não primitivos usados no AEM Forms é a `com.adobe.idp.Document` classe. Um documento geralmente é necessário para chamar uma operação AEM Forms. É principalmente um documento PDF, mas pode conter outros tipos de documentos, como SWF, HTML, XML ou um arquivo DOC. (Consulte [Transmissão de dados para serviços do AEM Forms usando a API](/help/forms/developing/invoking-aem-forms-using-java.md#passing-data-to-aem-forms-services-using-the-java-api)Java.)
...
* Quando o documento não estiver no servidor, use o servlet de upload Remoto para fazer upload de um documento para os AEM Forms. A novidade no AEM Forms é a capacidade de carregar documentos protegidos. Ao fazer upload de um documento seguro, é necessário usar um usuário que tenha a função Usuário *do aplicativo de upload de* Documentos. Sem essa função, o usuário não pode carregar um documento seguro. É recomendável usar logon único para fazer upload de um documento seguro. (Consulte [Passando documentos protegidos para chamar processos usando o recurso Remoto](invoking-aem-forms-using-remoting.md#passing-secure-documents-to-invoke-processes-using-remoting).)

>[!NOTE]
se o AEM Forms estiver configurado para permitir que documentos não protegidos sejam carregados, você poderá usar um usuário que não tenha a função Usuário do aplicativo de upload de Documentos para fazer upload de um documento. Um usuário também pode ter a permissão de Upload de Documento. No entanto, se os AEM Forms estiverem configurados para permitir apenas documentos protegidos, verifique se o usuário tem a função Usuário do aplicativo de upload de Documentos ou a permissão de Upload de Documentos. (Consulte [Configurar AEM Forms para aceitar documentos](invoking-aem-forms-using-remoting.md#configuring-aem-forms-to-accept-secure-and-unsecure-documents)seguros e não protegidos.

Use os recursos padrão de upload do Flash para o URL de upload designado: `https://SERVER:PORT/remoting/lcfileupload`. Você pode usar o `DocumentReference` objeto sempre que um parâmetro de entrada do tipo `Document` for esperado` private function startUpload():void  {  fileRef.addEventListener(Event.SELECT, selectHandler);  fileRef.addEventListener("uploadCompleteData", completeHandler);  try  {   var success:Boolean = fileRef.browse();  }    catch (error:Error)  {   trace("Unable to browse for files.");  }  }      private function selectHandler(event:Event):void {  var request:URLRequest = new  URLRequest("https://SERVER:PORT/remoting/lcfileupload")  try   {   fileRef.upload(request);   }    catch (error:Error)   {   trace("Unable to upload file.");   }  }    private function completeHandler(event:DataEvent):void  {   var params:Object = new Object();   var docRef:DocumentReference = new DocumentReference();   docRef.url = event.data as String;   docRef.referenceType = DocumentReference.REF_TYPE_URL;  }`. O Start Rápido Remoto usa o servlet de upload Remoto para passar um arquivo PDF para o `MyApplication/EncryptDocument`processo. (Consulte [Invocar um processo de duração curta transmitindo um documento não seguro usando (obsoleto para formulários AEM) AEM Forms Remota](invoking-aem-forms-using-remoting.md#invoking-a-short-lived-process-by-passing-an-unsecure-document-using-remoting).)
...
Para invocar um processo de AEM Forms de um aplicativo criado com o Flex, execute as seguintes tarefas:
...
>[!NOTE]
Esta seção discute como invocar um processo de AEM Forms e fazer upload de um documento quando o AEM Forms estiver configurado para fazer upload de documentos não protegidos. Para obter informações sobre como invocar processos de AEM Forms e fazer upload de documentos protegidos e como configurar AEM Forms para aceitar documentos seguros e não protegidos, consulte [Transmissão de documentos protegidos para chamar processos usando o Remoting](invoking-aem-forms-using-remoting.md#passing-secure-documents-to-invoke-processes-using-remoting).

**Criação de uma instância mx:RemoteObject**

Você cria uma `mx:RemoteObject` instância para invocar um processo de AEM Forms criado no Workbench. Para criar uma `mx:RemoteObject` instância, especifique os seguintes valores:

* **id:** O nome da `mx:RemoteObject` instância que representa o processo a ser chamado.
* **destino:** O nome do processo de AEM Forms a ser chamado. Por exemplo, para chamar o `MyApplication/EncryptDocument` processo, especifique `MyApplication/EncryptDocument`.
...
**Criar um Canal para AEM Forms**

Um aplicativo cliente pode invocar AEM Forms especificando um Canal em MXML ou ActionScript, como mostra o exemplo do ActionScript a seguir. O Canal deve ser um `AMFChannel`, `SecureAMFChannel`, `HTTPChannel`ou `SecureHTTPChannel`.
...
Os parâmetros de saída do processo de AEM Forms são desserializados em objetos ActionScript a partir dos quais o aplicativo cliente extrai parâmetros específicos por nome, como mostra o exemplo a seguir. (O valor de saída do `MyApplication/EncryptDocument` processo é nomeado `outDoc`.)
...
1. Configure uma `ChannelSet` instância para se comunicar com AEM Forms e associe-a à `mx:RemoteObject` instância. Consulte Criar um Canal para AEM Forms.
...
### Configuração de logon único em AEM Forms {#setting-up-single-sign-on-in-aem-forms}
...
Um aplicativo cliente acessa AEM Forms por meio de um terminal remoto usando o `RemoteObject` componente, como mostra o exemplo a seguir.
...
Um aplicativo criado com Flex inclui o cookie de autenticação com cada solicitação a um serviço AEM Forms. Por motivos de desempenho, o AEM Forms não valida o cookie em cada solicitação. No entanto, o AEM Forms detecta quando um cookie de autenticação é substituído por outro cookie de autenticação.
...
Para desconectar-se dos AEM Forms e invalidar uma sessão, o cookie de autenticação deve ser excluído do computador do cliente. Como o objetivo do logon único é permitir que um usuário faça logon uma vez, você não deseja que um aplicativo cliente exclua o cookie. Esta ação efetua logout do usuário.
...
>[!NOTE]
Para configurar AEM Forms para aceitar documentos não protegidos, selecione a opção Permitir upload de documentos não protegidos de aplicativos Flex. Em seguida, reinicie um aplicativo ou serviço para garantir que a configuração entre em vigor.
...
se o AEM Forms estiver configurado para permitir que documentos protegidos sejam carregados e o usuário não tiver a função Usuário *do aplicativo de upload do* Documento, uma exceção será lançada. Se o usuário tiver essa função, o arquivo será carregado e o processo será chamado.
...
### Mapeamento de tipos de dados complexos de AEM Forms {#mapping-aem-forms-complex-data-types}

Algumas operações de AEM Forms exigem tipos de dados complexos como valores de entrada. Esses tipos de dados complexos definem valores de tempo de execução usados pela operação. Por exemplo, a operação do Serviço de atendimento ao cliente exige uma `createCustomer` `Customer` instância que contenha valores de tempo de execução exigidos pelo serviço. Sem o tipo complexo, o Serviço de atendimento ao cliente lança uma exceção e não executa a operação.

Ao invocar um serviço AEM Forms, crie objetos ActionScript que mapeiam para tipos complexos AEM Forms necessários. Para cada tipo de dados complexo que uma operação requer, crie um objeto ActionScript separado.

Na classe ActionScript, use a tag de `RemoteClass` metadados para mapear para o tipo complexo de AEM Forms. Por exemplo, ao chamar a operação do Serviço de atendimento ao cliente, crie uma classe ActionScript que mapeie para o tipo de `createCustomer` `com.adobe.livecycle.sample.customer.Customer` dados.
...
O tipo de dados totalmente qualificado do tipo complexo de AEM Forms é atribuído à tag alias.

Os campos da classe ActionScript correspondem aos campos que pertencem ao tipo complexo AEM Forms. Os seis campos localizados na classe ActionScript do cliente correspondem aos campos que pertencem a `com.adobe.livecycle.sample.customer.Customer`.
...
A classe ActionScript do cliente pertence a um pacote chamado cliente. É recomendável colocar todas as classes ActionScript que mapeiam para tipos de dados AEM Forms complexos em seus próprios pacotes. Crie uma pasta na pasta src do projeto Flex e coloque o arquivo ActionScript na pasta, como mostrado na ilustração a seguir.
