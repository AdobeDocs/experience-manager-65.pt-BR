...
description: Saiba mais sobre as práticas recomendadas e recomendações de SEO em uma implementação de AEM.
topic-tags: managing
...
A SEO (Otimização do mecanismo de pesquisa) se tornou uma preocupação principal para muitos profissionais de marketing. Como resultado, as preocupações com a SEO devem ser abordadas em muitos projetos AEM.

Este documento descreve primeiramente alguns [Práticas recomendadas da SEO](#seo-best-practices) e recomendações sobre uma implementação AEM. Em seguida, ele detalha algumas [etapas de implementação mais complexas](#aem-configurations) abordadas na primeira seção.
...
Há algumas práticas recomendadas aceitas com URLs.
...
* Evite a utilização de parâmetros de consulta sempre que possível. Quando necessário, limite-os a dois ou menos.
   ...
   * Se uma estrutura de diretório não for uma opção, use seletores Sling no URL em vez de strings de consulta. Além do valor SEO que eles fornecem, os seletores sling também tornam as páginas acessíveis para o Dispatcher.

* Quanto mais legível por humanos for um URL, melhor. Ter palavras-chave presentes no URL aumenta o valor.
...
* Evite subdomínios sempre que possível, pois os mecanismos de pesquisa os tratam como entidades diferentes, fragmentando o valor SEO do site.
...
* Use URLs canônicos.
   ...
   * Inclua URLs canônicos no código do modelo de AEM.
...
* Certifique-se de que cada página seja distribuída somente de um protocolo.
   ...
   * Às vezes, os sites serão distribuídos pelo `http` até que um usuário chegue a uma página com, por exemplo, um formulário de check-out ou logon, no qual ele é alternado para `https`. Ao criar links a partir dessa página, se o usuário puder retornar para `http` páginas e acesse-as por meio de `https`, o mecanismo de pesquisa os rastreia como duas páginas separadas.

   * Atualmente, o Google prefere páginas `https` às páginas `http`. Eles ajudam a facilitar a vida de todos para distribuir todo o site `https`.
...
A tendência nos últimos anos tem sido remover parâmetros para tornar os URLs mais legíveis. Em várias plataformas, esse processo de remoção envolve a implementação de redirecionamentos no servidor da Web ou na Rede de entrega de conteúdo (CDN), mas o Sling torna o processo simples. Os seletores Sling:

* Melhoram a legibilidade do URL.
* Permitem armazenar suas páginas em cache no Dispatcher e melhorar a segurança.
* Permite abordar o conteúdo diretamente, em vez de ter um servlet genérico que recupera o conteúdo. Ela concede os benefícios das ACLs que você aplica ao repositório e aos filtros que você aplica ao Dispatcher.
...
Os servlets **Bin** seguem o padrão para o qual muitos desenvolvedores são usados na programação J2EE. O servlet é registrado em um caminho específico que, em AEM, normalmente está sob `/bin`e você extrai os parâmetros de solicitação necessários da string de consulta.
...
* Se quiser proteger este servlet, implemente sua própria lógica de segurança personalizada no servlet.
...
Nesse caso, o recurso que o URL endereça - uma instância do `myPageType` recurso - é acessível automaticamente no servlet. Para acessá-lo, você chama o seguinte:
...
* O Dispatcher já está configurado para veicular esse conteúdo como uma função de veiculação do site. Nenhuma configuração adicional é necessária.

### Regravação de URL {#url-rewriting}

No AEM, todas as páginas da Web são armazenadas no `/content/my-brand/my-content`. Embora esse local seja útil da perspectiva do gerenciamento de dados do repositório, não é necessariamente a forma como você deseja que seus clientes vejam seu site. Além disso, pode entrar em conflito com a orientação da SEO de manter os URLs o mais curtos possível. Além disso, você pode estar distribuindo vários sites da mesma instância do AEM e de nomes de domínio diferentes.
...
O desafio de localizar o nome da página é que muitas das ferramentas de localização disponíveis na plataforma de AEM dependem que os nomes das páginas correspondam às localidades para manter o conteúdo sincronizado.

O `sling:alias` A propriedade permite que você tenha Adobe bolo e coma também. Você pode adicionar `sling:alias` como uma propriedade de qualquer recurso para permitir um nome de alias para o recurso. No exemplo anterior, você teria o seguinte:
...
Esse fluxo permite que as ferramentas de tradução AEM, como o gerente de vários sites, continuem a manter uma relação entre:
...
Embora essa abordagem esteja documentada na documentação oficial do AEM e do Sling, o suporte regular à expressão fornecido por essa implementação é limitado no escopo quando comparado às opções disponíveis usando o `SlingResourceResolver` diretamente. Além disso, a implementação de mapeamentos desta maneira pode levar a problemas com a invalidação do cache do Dispatcher.
...
No entanto, também há uma maneira mais simples de gerenciar esse problema:
...
1. **Regras SlingResourceResolver**
   ...
   * **Apache Sling Resource Resolver Factory**
      ...
      `(org.apache.sling.jcr.resource.internal.JcrResourceResolverFactoryImpl)`.
   O Adobe recomenda criar os mapeamentos necessários para encurtar URLs como expressões regulares e, em seguida, definir essas configurações em um nó OsgiConfig, `config.publish` que está incluído na sua criação.
   ...
   * para apenas `/my-page.html`
   ...
   Essa conversão está em conformidade com a prática recomendada de manter os URLs o mais curtos possível.

1. **Mapeamento da saída de URL nas páginas**

   Depois de definir seus mapeamentos no Apache Sling Resource Resolver, use esses mapeamentos em seus componentes para garantir que os URLs enviados em suas páginas sejam curtos e organizados. Você pode realizar essa manutenção usando a função de mapa do `ResourceResolver`.
...
>[!NOTE]
>
>Disponível como recurso de produto desde a versão 6.5.11.0 do Adobe Experience Manager.
> 
>Para versões mais antigas, você pode registrar um Sling Servlet, para ouvir um `sitemap.xml` chame. Use o recurso fornecido por meio da API de servlet para pesquisar a página atual e seus descendentes para exibir um valor de `sitemap.xml` arquivo.
...
Na configuração padrão, a caixa de diálogo Propriedades da página fornece uma opção para marcar uma página como uma raiz do mapa de site e, portanto, conforme descrito acima, gerar um mapa de site próprio e seus descendentes. Esse comportamento é implementado pelas implementações da interface `SitemapGenerator` e pode ser estendido adicionando implementações alternativas. No entanto, como a frequência na qual os mapas de site XML são regenerados depende dos fluxos de trabalho e cargas de trabalho da criação de conteúdo, o produto não envia nenhum `SitemapScheduler` configuração. Dessa forma, ele torna o recurso efetivamente opcional.

Para habilitar o trabalho em segundo plano que gera os mapas de site XML como `SitemapScheduler` deve ser configurado. Para fazer isso, crie uma configuração OSGI para o PID `org.apache.sling.sitemap.impl.SitemapScheduler`. A expressão do scheduler `0 0 0 * * ?` pode ser usada como ponto de partida para regenerar todos os mapas de site XML uma vez por dia, à meia-noite.

![Apache Sling Sitemap - Scheduler](assets/sling-sitemap-scheduler.png)

O trabalho de geração de mapa do site pode ser executado em instâncias de nível de criação e publicação. Geralmente, é recomendável executar a geração em instâncias do nível de publicação, já que URLs canônicos adequados podem ser gerados somente lá (devido às regras de Mapeamento de recursos do Sling geralmente estarem presentes apenas em instâncias do nível de publicação). No entanto, é possível plug-in em uma implementação personalizada do mecanismo de externalização usado para gerar os URLs canônicos, implementando o [SitemapLinkExternalizer](https://javadoc.io/doc/com.adobe.cq.wcm/com.adobe.aem.wcm.seo/latest/com/adobe/aem/wcm/seo/sitemap/externalizer/SitemapLinkExternalizer.html) interface. Se uma implementação personalizada puder gerar os URLs canônicos de um mapa de site nas instâncias do nível de criação, a variável `SitemapScheduler` pode ser configurado para o modo de execução do autor. E a carga de trabalho de geração de mapa de site XML pode ser distribuída pelas instâncias do cluster de serviços de autor. Nesse cenário, deve-se ter cuidado ao manipular conteúdo que ainda não foi publicado, foi modificado ou está visível somente para um grupo restrito de usuários.

O AEM Sites contém uma implementação padrão de um `SitemapGenerator` que atravessa uma árvore de páginas para gerar um mapa de site. Ela é pré-configurada para produzir apenas os URLs canônicos de um site e qualquer alternativa de idioma que estiver disponível. Ela também pode ser configurada para incluir a data da última modificação de uma página, se necessário. Para fazer isso, ative a _Adicionar Última Modificação_ da _Adobe AEM SEO - Gerador de mapa de site da árvore de páginas_ Configuração e selecione um _Última Origem Modificada_. Quando os mapas do site são gerados no nível de publicação, é recomendável usar a data `cq:lastModified`.
